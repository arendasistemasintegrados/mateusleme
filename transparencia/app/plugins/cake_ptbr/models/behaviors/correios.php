...
class CorreiosBehavior extends ModelBehavior {
	 ...
	 *        	Código do serviço, ver as defines CORREIOS_*
	 * @param string $cepOrigem
	 *        	CEP de origem no formato XXXXX-XXX
	 * @param string $cepDestino
	 *        	CEP de destino no formato XXXXX-XXX
	 * @param float $peso
	 *        	Peso do pacote, em quilos
	 * @param boolean $maoPropria
	 *        	Usar recurso de mão própria?
	 * @param float $valorDeclarado
	 *        	Valor declarado do pacote
	...
	function valorFrete(&$model, $servico, $cepOrigem, $cepDestino, $peso, $maoPropria = false, $valorDeclarado = 0.0, $avisoRecebimento = false) {
		...
		$tipos = array (
				CORREIOS_SEDEX,
				CORREIOS_SEDEX_A_COBRAR,
				CORREIOS_SEDEX_10,
				CORREIOS_SEDEX_HOJE,
				CORREIOS_ENCOMENDA_NORMAL 
		);
	 ...
	 *
	 * @param string $url
	 *        	Caminho relativo da página nos Correios
	 * @param string $method
	 *        	Método de requisição (POST/GET)
	 * @param array $query
	 *        	Dados para enviar na página
