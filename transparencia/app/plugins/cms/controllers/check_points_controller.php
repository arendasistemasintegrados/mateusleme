...
class CheckPointsController extends CmsAppController {
	...
	public function restore() {
		...
		if (! is_dir ( $sDiretorio )) {
			...
			$this->Session->setFlash ( "Diretório das restaurações não configurado.", 'default', array (
		...
		if (! empty ( $this->data )) {
			...
			if ($this->Menu->restauraBackup ( $aRestore )) {
				$this->Session->setFlash ( "Restaurado com sucesso.", 'default', array (
						'class' => "alert alert-success" 
				) );
			} else {
				$this->Session->setFlash ( "Erro ao restaurar.", 'default', array (
						'class' => "alert alert-error" 
				) );
			}
		}
		...
		if (empty ( $aSnapshot )) {
			...
			$this->Session->setFlash ( "Nenhum ponto de restauração encontrado.", 'default', array (
					'class' => "alert alert-error" 
			) );
			$this->redirect ( array (
					'controller' => 'dashboard',
	...
	public function save() {
		$this->Session->setFlash ( "Não salvo.", 'default', array (
				'class' => "alert alert-error" 
		) );
		
		if ($this->CheckPoint->generate ()) {
			$this->Session->setFlash ( "Salvo com sucesso.", 'default', array (
					'class' => "alert alert-success" 
			) );
		}
