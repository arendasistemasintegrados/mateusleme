...
class Receita extends AppModel {
	 ...
	 * @param Integer $iMes
	 *        	- Mês a ser consultado
	 * @param Boolean $lRetornaDeducoes
	 *        	- Controla se deve retornar as deducoes em todas as consultas
	 *        	TRUE - Retorna as deducoes em todas as consultas
	 *        	FALSE - Retorna as deducoes apenas quando o estrutural for zero
	 * @return array
	 */
	public function getReceitasByEstrutural($sEstrutural = '', $iInstit = '', $iExercicio = '', $sWhere = '', $sOrderBy = '', $sLimit = '', $sWhereExterno = '', $iMes = 12, $lRetornaDeducoes = false) {
		...
		while ( $iNivel <= 15 ) {
			...
			foreach ( $aBuscaReceitas as $iIndice => $aDadosReceita ) {
				...
				$oStdDadosReceita->valor_diferenca = $oStdDadosReceita->previsao_inicial + $oStdDadosReceita->previsao_adicional - $oStdDadosReceita->valor_acumulado;
				
				/**
				 * ignoramos as receitas sem movimentação.
				 */
				if ($oStdDadosReceita->previsao_inicial == 0 && $oStdDadosReceita->previsao_adicional == 0 && $oStdDadosReceita->valor_periodo == 0 && $oStdDadosReceita->valor_acumulado == 0) {
