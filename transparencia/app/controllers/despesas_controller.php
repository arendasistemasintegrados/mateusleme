...
class DespesasController extends AppController {
	...
	var $helpers = array (
			'CakePtbr.Formatacao', // Helper de Formatação para padrão Português
			'Crumb', // Helper de criação da navegação do portal
			'JavaScript', // Helper de auxílio para javascript
			'Transparencia' 
	); // Helper de auxílio para a construção das telas do portal
	...
	function index() {
		...
		$aListaExercicios = $this->Empenho->find ( 'all', array (
				'fields' => 'DISTINCT Empenho.exercicio',
	...
	function getInstit() {
		...
		foreach ( $aListaInstituicoes as $iInd => $aInstituicao ) {
			...
			$oRetorno->rows [$iInd] ['cell'] = array (
					utf8_encode ( $sNomeInstituicao ),
					$nVlrEmpenhado,
					$nVlrAnulado,
					$nVlrLiquidado,
					$nVlrPago 
	...
	function getOrgao() {
		...
		foreach ( $aListaOrgaos as $iInd => $aOrgao ) {
			...
			$oRetorno->rows [$iInd] ['cell'] = array (
					utf8_encode ( $aOrgao [0] ['descricao'] ),
	...
	function getDotacaoEmpenho() {
		$oParametros = json_decode ( stripslashes ( $_POST ['aParametros'] ) );
		$aDadosEmpenho = $this->Empenho->find ( 'all', array (
				'conditions' => array (
						'Empenho.id' => $oParametros->iEmpenho 
				) 
		) );
		...
		$aRetonoDotacao = $this->Dotacao->find ( 'all', array (
				'conditions' => array (
						'Dotacao.id' => $iDotacao 
				) 
		) );
		...
		$oRetorno->rows [0] ['cell'] = array (
				'Orgão',
				$aDotacao ['Orgao'] ['codorgao'],
				utf8_encode ( $aDotacao ['Orgao'] ['descricao'] ) 
		);
		$oRetorno->rows [1] ['cell'] = array (
				'Unidade',
				$aDotacao ['Unidade'] ['codunidade'],
				utf8_encode ( $aDotacao ['Unidade'] ['descricao'] ) 
		);
		$oRetorno->rows [2] ['cell'] = array (
				'Função',
				$aDotacao ['Funcao'] ['codfuncao'],
				utf8_encode ( $aDotacao ['Funcao'] ['descricao'] ) 
		);
		$oRetorno->rows [3] ['cell'] = array (
				'Sub Função',
				$aDotacao ['SubFuncao'] ['codsubfuncao'],
				utf8_encode ( $aDotacao ['SubFuncao'] ['descricao'] ) 
		);
		$oRetorno->rows [4] ['cell'] = array (
				'Programa',
				$aDotacao ['Programa'] ['codprograma'],
				utf8_encode ( $aDotacao ['Programa'] ['descricao'] ) 
		);
		$oRetorno->rows [5] ['cell'] = array (
				'Projeto/Atividade',
				$aDotacao ['Projeto'] ['codprojeto'],
				utf8_encode ( $aDotacao ['Projeto'] ['descricao'] ) 
		);
		$oRetorno->rows [6] ['cell'] = array (
				'Elemento',
				$aDotacao ['Planoconta'] ['estrutural'],
				utf8_encode ( $aDotacao ['Planoconta'] ['descricao'] ) 
		);
		$oRetorno->rows [7] ['cell'] = array (
				'Recurso',
				$aDotacao ['Recurso'] ['codrecurso'],
				utf8_encode ( $aDotacao ['Recurso'] ['descricao'] ) 
		);
