...
class ReceitasController extends AppController {
	...
	function getInstit() {
		...
		foreach ( $aListaInstituicoes as $iInd => $aInstituicao ) {
			...
			$oRetorno->rows [$iInd] ['cell'] = array (
					utf8_encode ( $aInstituicao [0] ['descricao'] ),
	...
	public function getReceitas() {
		...
		$aListaReceitas = $this->Receita->getReceitasByEstrutural ( $oParametros->sEstrutural, $oParametros->iInstituicao, $oParametros->iExercicio, $sWhere, null, null, $sWhereExterno, $oParametros->iMes );
		
		$sOffSet = $iLimit * $iPages - $iLimit;
		
		$aListaReceitas = $this->Receita->getReceitasByEstrutural ( $oParametros->sEstrutural, $oParametros->iInstituicao, $oParametros->iExercicio, $sWhere, $sOrderBy, $iLimit . " offset $sOffSet ", $sWhereExterno, $oParametros->iMes, $lLiquidoDeducao );
	...
	public function getRecursos() {
		...
		$aListaRecursos = $this->Recurso->getReceitas ( $sWhere, null, null, $oParametros->iExercicio );
		...
		$aListaRecursos = $this->Recurso->getReceitas ( $sWhere, $sOrderBy, $iLimit . " offset $sOffSet ", $oParametros->iExercicio, $oParametros->iMes );
		
		foreach ( $aListaRecursos as $iInd => $oRecuros ) {
			
			$oRetorno->rows [$iInd] ['id'] = $oRecuros->id;
			$oRetorno->rows [$iInd] ['cell'] = array (
					utf8_encode ( $oRecuros->descricao ),
