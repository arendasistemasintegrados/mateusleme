...
class DbConfigTask extends Shell {
	...
	function __interactive() {
		...
		while ( $done == false ) {
			...
			$persistent = $this->in ( 'Persistent Connection?', array (
					'y',
					'n' 
			), 'n' );
			...
			while ( $password == '' && $blankPassword == false ) {
				...
				if ($password == '') {
					$blank = $this->in ( 'The password you supplied was empty. Use an empty password?', array (
							'y',
							'n' 
					), 'n' );
					if ($blank == 'y') {
	...
	function __verify($config) {
		...
		$this->hr ();
		$looksGood = $this->in ( 'Look okay?', array (
				'y',
				'n' 
		), 'y' );
