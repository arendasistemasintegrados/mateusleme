...
class ExtractTask extends Shell {
	...
	function execute() {
		...
		if (isset ( $this->params ['merge'] )) {
			...
			$this->out ();
			$response = $this->in ( sprintf ( __ ( 'Would you like to merge all domains strings into the default.pot file?', true ) ), array (
					'y',
					'n' 
			), 'n' );
	...
	function __extractTokens() {
		foreach ( $this->__files as $file ) {
			...
			unset ( $allTokens );
			$this->__parse ( '__', array (
					'singular' 
			) );
			$this->__parse ( '__n', array (
					'singular',
					'plural' 
			) );
			$this->__parse ( '__d', array (
					'domain',
					'singular' 
			) );
			$this->__parse ( '__c', array (
					'singular' 
			) );
			$this->__parse ( '__dc', array (
					'domain',
					'singular' 
			) );
			$this->__parse ( '__dn', array (
					'domain',
					'singular',
					'plural' 
			) );
			$this->__parse ( '__dcn', array (
					'domain',
					'singular',
					'plural' 
			) );
	...
	function __parse($functionName, $map) {
		...
		while ( ($tokenCount - $count) > 1 ) {
			list ( $countToken, $firstParenthesis ) = array (
					$this->__tokens [$count],
					$this->__tokens [$count + 1] 
			);
	...
	function __writeFiles() {
		...
		foreach ( $this->__storage as $domain => $sentences ) {
			...
			while ( $overwriteAll === false && $File->exists () && strtoupper ( $response ) !== 'Y' ) {
				$this->out ();
				$response = $this->in ( sprintf ( __ ( 'Error: %s already exists in this location. Overwrite? [Y]es, [N]o, [A]ll', true ), $filename ), array (
						'y',
						'n',
						'a' 
				), 'y' );
