...
class TestSuiteShell extends Shell {
	 ...
	 */
	function __canRun() {
		$isNeitherAppNorCore = ! in_array ( $this->category, array (
				'app',
				'core' 
		) );
		...
		if ($isNeitherAppNorCore && ! $isPlugin) {
			$message = sprintf ( __ ( '%s is an invalid test category (either "app", "core" or name of a plugin)', true ), $this->category );
			$this->error ( $message );
			return false;
		}
		...
		$message = sprintf ( __ ( '%s %s %s is an invalid test identifier', true ), $this->category, $this->type, $this->file );
		$this->err ( $message );
		return false;
	}
	/**
	 ...
	 *        	The category of the test. Either 'app', 'core' or a plugin name.
