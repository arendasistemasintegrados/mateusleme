...
class Router {
	...
	/**
	 * List of valid extensions to parse from a URL.
	 ...
	 * Includes compatibility for existing Routing.admin
	 * configurations.
	 *
	 * @return void
	 * @access private
	...
	/**
	 * Specifies what named parameters CakePHP should be parsing.
	...
	function mapResources($controller, $options = array()) {
		...
		$options = array_merge ( array (
				'prefix' => '/',
				'id' => $self->__named ['ID'] . '|' . $self->__named ['UUID'] 
		), $options );
	...
	function __connectDefaultRoutes() {
		if ($plugins = App::objects ( 'plugin' )) {
			...
			$this->connect ( '/:plugin', array (
					'action' => 'index' 
			), $shortParams );
			$this->connect ( '/:plugin/:controller', array (
					'action' => 'index' 
			), $match );
	 ...
	 *
	 * @param $which A
	 *        	zero-based array index representing the route to move. For example,
	...
	function _handleNoRoute($url) {
		...
		list ( $args, $named ) = array (
				Set::filter ( $args, true ),
				Set::filter ( $named, true ) 
		);
		foreach ( $this->__prefixes as $prefix ) {
			if (! empty ( $url [$prefix] )) {
	 ...
	 * Works similarly to Router::url(), but
	 ...
	 *
	 * @param array $param
	 *        	The params array that needs to be reversed.
	...
	function reverse($params) {
		...
		$named = $params ['named'];
		$url = $params ['url'];
		unset ( $params ['pass'], $params ['named'], $params ['paging'], $params ['models'], $params ['url'], $url ['url'], $params ['autoRender'], $params ['bare'], $params ['requested'], $params ['return'] );
...
class CakeRoute {
	 ...
	 * @param string $params
	 *        	Array of parameters and additional options for the Route
	 * @return void
	 * @access public
	 */
	 ...
	 * Uses the template, defaults and options
	 ...
	 * Persistant parameters are a special
