...
class MagicDb extends Object {
	 ...
	 *
	 * @param string $data
	 *        	A MagicDb string to turn into an array
	...
	function toArray($data = null) {
		...
		$validHeader = count ( $lines ) > 3 && preg_match ( '/^# Date:([0-9]{4}-[0-9]{2}-[0-9]{2})$/', $lines [1], $date ) && preg_match ( '/^# Source:(.+)$/', $lines [2], $source ) && strlen ( $lines [3] ) == 0;
		if (! $validHeader) {
			return $db;
		}
	 ...
	 *
	 * @param mixed $magicDb
	 *        	A $magicDb string / array to validate (optional)
