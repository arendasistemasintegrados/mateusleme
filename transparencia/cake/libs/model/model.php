...
class Model extends Overloadable {
	 ...
	 * Append entries for validation as ('field_name' => '/^perl_compat_regexp$/')
	...
	function __construct($id = false, $table = null, $ds = null) {
		...
		if ($this->useTable !== false) {
			...
			if ($this->displayField == null) {
				$this->displayField = $this->hasField ( array (
						'title',
						'name',
						$this->primaryKey 
				) );
	...
	/**
	 * Sets a custom table for your controller class.
	...
	function setSource($tableName) {
		...
		if ($db->isInterfaceSupported ( 'listSources' )) {
			...
			if (is_array ( $sources ) && ! in_array ( strtolower ( $this->tablePrefix . $tableName ), array_map ( 'strtolower', $sources ) )) {
				return $this->cakeError ( 'missingTable', array (
						array (
	...
	function save($data = null, $validate = true, $fieldList = array()) {
		...
		if ($options ['callbacks'] === true || $options ['callbacks'] === 'before') {
			$result = $this->Behaviors->trigger ( $this, 'beforeSave', array (
					$options 
			), array (
					'break' => true,
					'breakOn' => false 
	...
	function __saveMulti($joined, $id, &$db) {
		foreach ( $joined as $assoc => $data ) {
			...
			if (isset ( $this->hasAndBelongsToMany [$assoc] )) {
				...
				$isUUID = ! empty ( $this->{$join}->primaryKey ) && ($this->{$join}->_schema [$this->{$join}->primaryKey] ['length'] == 36 && ($this->{$join}->_schema [$this->{$join}->primaryKey] ['type'] === 'string' || $this->{$join}->_schema [$this->{$join}->primaryKey] ['type'] === 'binary'));
				...
				if (isset ( $associations [$association] )) {
					...
					switch ($type) {
						...
						case 'hasMany' :
							...
							$_options = array_merge ( $options, array (
									'atomic' => false 
							) );
	...
	function delete($id = null, $cascade = true) {
		...
		if ($this->beforeDelete ( $cascade )) {
			$filters = $this->Behaviors->trigger ( $this, 'beforeDelete', array (
					$cascade 
			), array (
					'break' => true,
					'breakOn' => false 
	...
	function exists() {
		...
		$conditions = array (
				$this->alias . '.' . $this->primaryKey => $this->getID () 
		);
		$query = array (
				'conditions' => $conditions,
				'recursive' => - 1,
	 ...
	 *
	 * @param array $conditions
	 *        	SQL conditions array, or type of find operation (all / first / count /
	 *        	neighbors / list / threaded)
	 * @param mixed $fields
	 *        	Either a single string of a field name, or an array of field names, or
	...
	function find($conditions = null, $fields = array(), $order = null, $recursive = null) {
		if (! is_string ( $conditions ) || (is_string ( $conditions ) && ! array_key_exists ( $conditions, $this->_findMethods ))) {
			$type = 'first';
			$query = array_merge ( compact ( 'conditions', 'fields', 'order', 'recursive' ), array (
					'limit' => 1 
			) );
		} else {
			list ( $type, $query ) = array (
					$conditions,
					$fields 
			);
		}
		...
		if ($query ['callbacks'] === true || $query ['callbacks'] === 'before') {
			$return = $this->Behaviors->trigger ( $this, 'beforeFind', array (
					$query 
			), array (
					'break' => true,
					'breakOn' => false,
	...
	function __filterResults($results, $primary = true) {
		$return = $this->Behaviors->trigger ( $this, 'afterFind', array (
				$results,
				$primary 
		), array (
				'modParams' => true 
	 ...
	 * @param mixed $with
	 *        	The 'with' key of the model association
	 * @param array $keys
