...
class TreeBehavior extends ModelBehavior {
	...
	function beforeDelete(&$Model) {
		...
		if ($diff > 2) {
			...
			$scope [] ["{$Model->alias}.{$left} BETWEEN ? AND ?"] = array (
					$data [$left] + 1,
					$data [$right] - 1 
			);
			...
			if ($missingParents) {
				if ($missingParentAction == 'return') {
					...
					$Model->deleteAll ( array (
							$Model->primaryKey => array_flip ( $missingParents ) 
					) );
				} else {
	...
	function __sync(&$Model, $shift, $dir = '+', $conditions = array(), $created = false, $field = 'both') {
		...
		$Model->updateAll ( array (
				$Model->alias . '.' . $field => $Model->escapeField ( $field ) . ' ' . $dir . ' ' . $shift 
		), $conditions );
