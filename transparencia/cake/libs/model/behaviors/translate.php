...
class TranslateBehavior extends ModelBehavior {
	...
	function setup(&$model, $config = array()) {
		...
		if (! $db->connected) {
			trigger_error ( sprintf ( __ ( 'Datasource %s for TranslateBehavior of model %s is not connected', true ), $model->useDbConfig, $model->alias ), E_USER_ERROR );
			return false;
		}
	...
	function bindTranslation(&$model, $fields, $reset = true) {
		...
		foreach ( $fields as $key => $value ) {
			...
			if (array_key_exists ( $field, $this->settings [$model->alias] )) {
				...
				$this->settings [$model->alias] = array_merge ( array_diff_assoc ( $this->settings [$model->alias], array (
						$field 
				) ) );
			}
			...
			if (array_key_exists ( $field, $this->runtime [$model->alias] ['fields'] )) {
				...
				$this->runtime [$model->alias] ['fields'] = array_merge ( array_diff_assoc ( $this->runtime [$model->alias] ['fields'], array (
						$field 
				) ) );
			}
			...
			if (is_null ( $association )) {
				...
				foreach ( array (
					...
					if (isset ( $model->{$type} [$association] ) || isset ( $model->__backAssociation [$type] [$association] )) {
						trigger_error ( sprintf ( __ ( 'Association %s is already binded to model %s', true ), $association, $model->alias ), E_USER_ERROR );
						return false;
					}
				}
