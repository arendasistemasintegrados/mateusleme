...
class ModelBehavior extends Object {
	...
	/**
	 * After find callback.
	 * Can be used to modify any results returned by find and findAll.
	 *
	 * @param object $model
	 *        	Model using this behavior
	 * @param mixed $results
...
class BehaviorCollection extends Object {
	...
	function attach($behavior, $config = array()) {
		...
		if (! App::import ( 'Behavior', $behavior )) {
			$this->cakeError ( 'missingBehaviorFile', array (
					array (
							'behavior' => $behavior,
		...
		if (! class_exists ( $class )) {
			$this->cakeError ( 'missingBehaviorClass', array (
					array (
							'behavior' => $class,
		...
		foreach ( $this->{$name}->mapMethods as $method => $alias ) {
			$this->__mappedMethods [$method] = array (
					$alias,
					$name 
			);
		}
		...
		foreach ( $methods as $m ) {
			if (! isset ( $parentMethods [$m] )) {
				$methodAllowed = ($m [0] != '_' && ! array_key_exists ( $m, $this->__methods ) && ! in_array ( $m, $callbacks ));
				if ($methodAllowed) {
					$this->__methods [$m] = array (
							$m,
							$name 
					);
				}
	...
	/**
	 * Disables callbacks on a behavior or array of behaviors.
	 * Public behavior methods are still
	 * callable as normal.
	 *
	 * @param mixed $name
