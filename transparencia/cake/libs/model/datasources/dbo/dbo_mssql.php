...
class DboMssql extends DboSource {
	...
	function fields(&$model, $alias = null, $fields = array(), $quote = true) {
		...
		if ($count >= 1 && strpos ( $fields [0], 'COUNT(*)' ) === false && strpos ( $fields [0], 'COUNT(DISTINCT' ) === false) {
			$result = array ();
			for($i = 0; $i < $count; $i ++) {
				$prepend = '';
		...
		switch (strtolower ( $type )) {
			case 'select' :
				...
				if (preg_match ( '/offset\s+([0-9]+)/i', $limit, $offset )) {
					...
					list ( $order2, $rOrder ) = array (
							$this->__mapFields ( $order ),
							$this->__mapFields ( $rOrder ) 
					);
	...
	function buildColumn($column) {
		...
		} else if (array_keys ( $column ) == array (
				'type',
				'name' 
		)) {
			$result .= ' NULL';
