...
class DboSource extends DataSource {
	...
	/**
	 * Output information about an SQL query.
	 * The SQL statement, number of rows in resultset,
				...
				for($j = 0; $j < $count2; $j ++) {
					...
					if ($model->alias != $className && ! in_array ( $className, $filtered )) {
						...
						if (isset ( $model->{$className} ) && is_object ( $model->{$className} )) {
							$data = $model->{$className}->afterFind ( array (
									array (
	...
	function generateAssociationQuery(&$model, &$linkModel, $type, $association = null, $assocData = array(), &$queryData, $external = false, &$resultSet) {
		...
		if (empty ( $queryData ['fields'] )) {
			...
			$assocFields = $this->fields ( $model, $model->alias, array (
		...
		switch ($type) {
			case 'hasOne' :
			case 'belongsTo' :
				$conditions = $this->__mergeConditions ( $assocData ['conditions'], $this->getConstraint ( $type, $model, $linkModel, $alias, array_merge ( $assocData, compact ( 'external', 'self' ) ) ) );
			...
			case 'hasMany' :
				...
				if (! empty ( $assocData ['foreignKey'] )) {
					$assocData ['fields'] = array_merge ( $assocData ['fields'], $this->fields ( $linkModel, $alias, array (
							"{$alias}.{$assocData['foreignKey']}" 
					) ) );
				}
				$query = array (
	 ...
	 * @return boolean TRUNCATE TABLE statement, false if not applicable.
	 * @access public
	 */
	...
	function __quoteFields($conditions) {
		...
		$conditions = preg_replace_callback ( '/(?:[\'\"][^\'\"\\\]*(?:\\\.[^\'\"\\\]*)*[\'\"])|([a-z0-9_' . $start . $end . ']*\\.[a-z0-9_' . $start . $end . ']*)/i', array (
				&$this,
				'__quoteMatchedField' 
		), $conditions );
