...
class AuthComponent extends Object {
	 ...
	 * $userModel, i.e.
	 * array('username' => 'login_name', 'password' => 'passwd').
	 *
	 * @var array
	 * @access public
	 ...
	 * ACO nodes is computed.
	 * If, for example, all controller nodes are nested
	...
	/**
	 * Error to display when user login fails.
	 * For security purposes, only one error is used for all
	 ...
	 * Used for controller-based validation ($validate = 'controller').
	 *
	 * @var array
	 * @access public
	 ...
	 * Used for controller-based authentication.
	 *
	 * @param array $map
	 *        	Actions to map
	 * @return void
	 ...
	 * The $data provided can be any data
	...
	function user($key = null) {
		...
		if ($key == null) {
			$model = & $this->getModel ();
			return array (
					$model->alias => $this->Session->read ( $this->sessionKey ) 
			);
		} else {
	 ...
	 * @param mixed $user
	 *        	Optional. The identity of the user to be validated.
	 ...
	 *
	 * @param string $action
	 *        	Optional. The controller/action path to validate the
	...
	function action($action = ':plugin/:controller/:action') {
		...
		return str_replace ( array (
				...
				Inflector::camelize ( $this->params ['controller'] ),
				$this->params ['action'],
				$plugin 
