...
class EmailComponent extends Object {
	...
	function _render($content) {
		...
		if ($this->sendAs === 'both') {
			...
			$content = $View->element ( 'email' . DS . 'text' . DS . $this->template, array (
					'content' => $content 
			), true );
			...
			$htmlContent = $View->element ( 'email' . DS . 'html' . DS . $this->template, array (
					'content' => $htmlContent 
			), true );
		...
		$content = $View->element ( 'email' . DS . $this->sendAs . DS . $this->template, array (
				'content' => $content 
		), true );
	 ...
	 * @access public
	 * @param
	 *        	array Associative array containing headers to be set.
	 */
	...
	function _createHeader() {
		...
		if ($this->delivery == 'smtp') {
			$headers ['To'] = implode ( ', ', array_map ( array (
					$this,
					'_formatAddress' 
			), ( array ) $this->to ) );
		}
		...
		if (! empty ( $this->cc )) {
			$headers ['Cc'] = implode ( ', ', array_map ( array (
					$this,
					'_formatAddress' 
			), ( array ) $this->cc ) );
		}
		...
		if (! empty ( $this->bcc ) && $this->delivery != 'smtp') {
			$headers ['Bcc'] = implode ( ', ', array_map ( array (
					$this,
					'_formatAddress' 
			), ( array ) $this->bcc ) );
		}
