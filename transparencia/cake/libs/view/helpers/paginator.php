...
class PaginatorHelper extends AppHelper {
	 ...
	 *        	Title for the link.
	 * @param string $key
	 *        	The name of the key that the recordset should be sorted. If $key is null
	...
	function link($title, $url = array(), $options = array()) {
		...
		$url = array_merge ( array (
				'page' => $this->current ( $model ) 
		), $url );
		$url = array_merge ( Set::filter ( $url, true ), array_intersect_key ( $url, array (
				'plugin' => true 
		) ) );
