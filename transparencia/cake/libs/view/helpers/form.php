...
class FormHelper extends AppHelper {
	 ...
	 * to validation, field length and field type.
	 * Appends information into
	 * $this->fieldset.
	 *
	...
	function create($model = null, $options = array()) {
		...
		if (isset ( $this->fieldset [$modelEntity] ['key'] )) {
			...
			$recordExists = (isset ( $this->data [$model] ) && ! empty ( $this->data [$model] [$data ['key']] ) && ! is_array ( $this->data [$model] [$data ['key']] ));
	...
	function label($fieldName = null, $text = null, $options = array()) {
		...
		return sprintf ( $this->Html->tags ['label'], $labelFor, $this->_parseAttributes ( $options ), $text );
	}
	...
	function inputs($fields = null, $blacklist = null) {
		...
		if ($legend === true) {
			...
			$isEdit = (strpos ( $this->action, 'update' ) !== false || strpos ( $this->action, 'edit' ) !== false);
			if ($isEdit) {
	...
	function checkbox($fieldName, $options = array()) {
		...
		return $output . sprintf ( $this->Html->tags ['checkbox'], $options ['name'], $this->_parseAttributes ( $options, array (
				'name' 
		), null, ' ' ) );
	}
	...
	function radio($fieldName, $options = array(), $attributes = array()) {
		...
		foreach ( $options as $optValue => $optTitle ) {
			...
			$parsedOptions = $this->_parseAttributes ( array_merge ( $attributes, $optionsHere ), array (
					'name',
					'type',
					'id' 
			), '', ' ' );
	...
	function text($fieldName, $options = array()) {
		$options = $this->_initInputField ( $fieldName, array_merge ( array (
				'type' => 'text' 
		), $options ) );
		return sprintf ( $this->Html->tags ['input'], $options ['name'], $this->_parseAttributes ( $options, array (
				'name' 
		), null, ' ' ) );
	}
	...
	function password($fieldName, $options = array()) {
		...
		return sprintf ( $this->Html->tags ['password'], $options ['name'], $this->_parseAttributes ( $options, array (
				'name' 
		), null, ' ' ) );
	}
	...
	function textarea($fieldName, $options = array()) {
		...
		return sprintf ( $this->Html->tags ['textarea'], $options ['name'], $this->_parseAttributes ( $options, array (
				'type',
				'name' 
		), null, ' ' ), $value );
	}
	...
	function hidden($fieldName, $options = array()) {
		...
		return sprintf ( $this->Html->tags ['hidden'], $options ['name'], $this->_parseAttributes ( $options, array (
				'name',
				'class' 
		), '', ' ' ) );
	}
	...
	function file($fieldName, $options = array()) {
		...
		foreach ( array (
			...
			$this->__secure ( array_merge ( $field, array (
					$suffix 
			) ) );
		}
	...
	function button($title, $options = array()) {
		...
		return sprintf ( $this->Html->tags ['button'], $options ['type'], $this->_parseAttributes ( $options, array (
				'type' 
		), ' ', '' ), $title );
	}
	...
	function submit($caption = null, $options = array()) {
		...
		if (strpos ( $caption, '://' ) !== false) {
			unset ( $options ['type'] );
			$out .= $before . sprintf ( $this->Html->tags ['submitimage'], $caption, $this->_parseAttributes ( $options, null, '', ' ' ) ) . $after;
			...
			$out .= $before . sprintf ( $this->Html->tags ['submitimage'], $this->assetTimestamp ( $url ), $this->_parseAttributes ( $options, null, '', ' ' ) ) . $after;
		} else {
			$options ['value'] = $caption;
			$out .= $before . sprintf ( $this->Html->tags ['submit'], $this->_parseAttributes ( $options, null, '', ' ' ) ) . $after;
		}
	 ...
	 *        	The option selected by default. If null, the default value
	...
	function select($fieldName, $options = array(), $selected = null, $attributes = array()) {
		...
		if (! empty ( $tag ) || isset ( $template )) {
			...
			$select [] = sprintf ( $tag, $attributes ['name'], $this->_parseAttributes ( $attributes, array (
					'name',
					'value' 
			) ) );
		}
		...
		$select = array_merge ( $select, $this->__selectOptions ( array_reverse ( $options, true ), $selected, array (), $showParents, array (
	...
	function year($fieldName, $minYear = null, $maxYear = null, $selected = null, $attributes = array()) {
		...
		return $this->select ( $fieldName . '.year', $this->__generateOptions ( 'year', $yearOptions ), $selected, $attributes );
	}
	...
	function month($fieldName, $selected = null, $attributes = array()) {
		...
		return $this->select ( $fieldName . ".month", $this->__generateOptions ( 'month', array (
	...
	function hour($fieldName, $format24Hours = false, $selected = null, $attributes = array()) {
		...
		return $this->select ( $fieldName . ".hour", $this->__generateOptions ( $format24Hours ? 'hour24' : 'hour' ), $selected, $attributes );
	}
	...
	function minute($fieldName, $selected = null, $attributes = array()) {
		...
		return $this->select ( $fieldName . ".min", $this->__generateOptions ( 'minute', $minuteOptions ), $selected, $attributes );
	}
	...
	function meridian($fieldName, $selected = null, $attributes = array()) {
		...
		return $this->select ( $fieldName . ".meridian", $this->__generateOptions ( 'meridian' ), $selected, $attributes );
	}
	...
	function dateTime($fieldName, $dateFormat = 'DMY', $timeFormat = '12', $selected = null, $attributes = array()) {
		...
		foreach ( preg_split ( '//', $dateFormat, - 1, PREG_SPLIT_NO_EMPTY ) as $char ) {
			switch ($char) {
				case 'Y' :
					$selects [] = $this->year ( $fieldName, $minYear, $maxYear, $year, $selectYearAttr );
					break;
				case 'M' :
		...
		switch ($timeFormat) {
			case '24' :
				$opt .= $this->hour ( $fieldName, true, $hour, $selectHourAttr ) . ':' . $this->minute ( $fieldName, $min, $selectMinuteAttr );
				break;
			case '12' :
				$opt .= $this->hour ( $fieldName, false, $hour, $selectHourAttr ) . ':' . $this->minute ( $fieldName, $min, $selectMinuteAttr ) . ' ' . $this->meridian ( $fieldName, $meridian, $selectMeridianAttr );
				break;
			default :
				$opt .= '';
				break;
	...
	function __selectOptions($elements = array(), $selected = null, $parents = array(), $showParents = null, $attributes = array()) {
		...
		foreach ( $elements as $name => $title ) {
			...
			if ($name !== null) {
				if ((! $selectedIsArray && ! $selectedIsEmpty && ( string ) $selected == ( string ) $name) || ($selectedIsArray && in_array ( $name, $selected ))) {
					if ($attributes ['style'] === 'checkbox') {
				...
				if ($showParents || (! in_array ( $title, $parents ))) {
					...
					if ($attributes ['style'] === 'checkbox') {
						...
						$tagName = Inflector::camelize ( $this->model () . '_' . $this->field () . '_' . Inflector::slug ( $name ) );
						...
						$item = sprintf ( $this->Html->tags ['checkboxmultiple'], $name, $this->_parseAttributes ( $htmlOptions ) );
