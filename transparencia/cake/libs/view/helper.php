...
class Helper extends Overloadable {
	...
	function _parseAttributes($options, $exclude = null, $insertBefore = ' ', $insertAfter = null) {
		if (is_array ( $options )) {
			...
			$keys = array_diff ( array_keys ( $options ), array_merge ( $exclude, array (
					'escape' 
			) ) );
	...
	function setEntity($entity, $setScope = false) {
		...
		if (ClassRegistry::isKeySet ( $model )) {
			...
			for($i = 0; $i < $count; $i ++) {
				if (is_a ( $ModelObj, 'Model' ) && ($ModelObj->hasField ( $parts [$i] ) || array_key_exists ( $parts [$i], $ModelObj->validate ))) {
					$hasField = $i;
	...
	/**
	 * Returns null if given FORM field has no errors.
	 * Otherwise it returns the constant set in
	...
	function domId($options = null, $id = 'id') {
		...
		$dom = $model . join ( '', array_map ( array (
				'Inflector',
				'camelize' 
		), $entity ) );
	...
	function __clean() {
		...
		$this->__cleaned = str_replace ( array (
				"&amp;",
				"&lt;",
				"&gt;" 
		), array (
