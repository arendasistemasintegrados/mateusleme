...
class View extends Object {
	...
	/**
	 * Turns on or off Cake's conventional mode of rendering views.
	 * On by default.
	 *
	 * @var boolean
	 * @access public
	...
	/**
	 * Turns on or off Cake's conventional mode of finding layout files.
	 * On by default.
	 *
	 * @var boolean
	 * @access public
	...
	function render($action = null, $layout = null, $file = null) {
		...
		if ($out !== false) {
			if ($layout && $this->autoLayout) {
				...
				$isCached = (isset ( $this->loaded ['cache'] ) || Configure::read ( 'Cache.check' ) === true);
	 ...
	 * All helpers must implement this method,
	...
	function entity() {
		...
		if (! empty ( $this->entityPath )) {
			...
			$count = count ( $path );
			if (($count == 1 && ! empty ( $this->association )) || ($count == 1 && $this->model != $this->entityPath) || ($count == 1 && empty ( $this->association ) && ! empty ( $this->field )) || ($count == 2 && ! empty ( $this->fieldSuffix )) || is_numeric ( $path [0] ) && ! empty ( $assoc )) {
	 ...
	 * Uses layouts/error.ctp to render the page.
	 *
	 * @param integer $code
	...
	function error($code, $name, $message) {
		...
		print ($this->_render ( $this->_getLayoutFileName ( 'error' ), array (
				'code' => $code,
				'name' => $name,
				'message' => $message 
		) )) ;
				...
				if (! class_exists ( $helperCn )) {
					...
					if (! $isLoaded) {
						if (! App::import ( 'Helper', $helper )) {
							$this->cakeError ( 'missingHelperFile', array (
									array (
											'helper' => $helper,
					...
					if (! class_exists ( $helperCn )) {
						$this->cakeError ( 'missingHelperClass', array (
								array (
										'helper' => $helper,
										'file' => Inflector::underscore ( $helper ) . '.php',
										'base' => $this->base 
	 ...
	 *        	the filename that should exist
	 * @return false
	 * @access protected
	 */
