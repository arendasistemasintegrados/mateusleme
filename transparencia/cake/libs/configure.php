...
class Configure extends Object {
	 ...
	 *        	name of file to load, extension must be .php and only the name
		...
		if ($boot) {
			...
			if (Configure::read ( 'Cache.disable' ) !== true) {
				...
				if (empty ( $cache ['settings'] )) {
					...
					$cache = Cache::config ( 'default', array (
							'engine' => 'File' 
					) );
				}
...
class App extends Object {
	...
	function build($paths = array(), $reset = false) {
		...
		foreach ( $defaults as $type => $default ) {
			...
			if (! empty ( $paths [$type] )) {
				$path = array_flip ( array_flip ( array_merge ( ( array ) $paths [$type], $_this->{$type}, $merge ) ) );
	...
	/**
	 * Get the path that a plugin is on.
	...
	/**
	 * Find the path that a theme is on.
	 * Search through the defined theme paths.
	 *
	 * @param string $theme
	 ...
	 *
	 * @param string $type
	 *        	valid values are: 'model', 'behavior', 'controller', 'component',
	 ...
	 * @param mixed $path
	 *        	Optional Scan only the path given. If null, paths for the chosen
	 *        	type will be used.
	 * @param boolean $cache
