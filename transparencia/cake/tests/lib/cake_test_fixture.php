...
class CakeTestFixture extends Object {
	 ...
	 *
	 * @param
	 *        	object Cake's DBO driver (e.g: DboMysql).
	 * @access public
	 *        
	 */
	function init() {
	 ...
	 *
	 * @param object $db
	 *        	of the database object used to create the fixture table
	...
	function create(&$db) {
		...
		$this->Schema->_build ( array (
				$this->table => $this->fields 
		) );
		return ($db->execute ( $db->createSchema ( $this->Schema ), array (
				'log' => false 
		) ) !== false);
	}
	...
	function drop(&$db) {
		...
		return ($db->execute ( $db->dropSchema ( $this->Schema ), array (
				'log' => false 
		) ) !== false);
	}
	...
	function insert(&$db) {
		if (! isset ( $this->_insert )) {
			...
			if (isset ( $this->records ) && ! empty ( $this->records )) {
				...
				foreach ( $this->records as $record ) {
					$recordValues = array ();
					foreach ( array_merge ( $default, array_map ( array (
							&$db,
							'value' 
					), $record ) ) as $value ) {
	 ...
	 * Can be overwritten by classes extending CakeFixture to trigger other events before / after
	 * truncate.
	 *
	 * @param object $db
