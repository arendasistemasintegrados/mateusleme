...
class CodeCoverageManager {
	 ...
	 *        	The test case file being covered.
	 * @param object $reporter
	 *        	Instance of the reporter running.
	 * @return void
	 * @static
	 *
	 */
	...
	/**
	 * Stops/pauses code coverage collection.
	 * Does not clean the
	...
	/**
	 * Clears the existing code coverage information.
	 * Also stops any
	 * running collection.
	 *
	 * @return void
	...
	function __testObjectFileFromCaseFile($file, $isApp = true) {
		...
		$testFile = str_replace ( array (
				'/',
				$testManager->_testExtension 
		), array (
				DS,
				'.php' 
		), $file );
	...
	function __getExecutableLines($content) {
		...
		$content = preg_replace_callback ( "/(@codeCoverageIgnoreStart.*?@codeCoverageIgnoreEnd)/is", array (
				'CodeCoverageManager',
	...
	function __calcCoverage($lineCount, $coveredCount) {
		...
		return ($lineCount != 0) ? round ( 100 * $coveredCount / $lineCount, 2 ) : '0.00';
	}
