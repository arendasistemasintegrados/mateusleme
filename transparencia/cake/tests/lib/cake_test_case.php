...
class CakeTestCase extends UnitTestCase {
	 ...
	 *
	 * @param Controller $controller
	 *        	that's about to be invoked.
	 * @param array $params
	 *        	parameters as sent by testAction().
	 * @return void
	 * @access public
	 */
	function startController(&$controller, $params = array()) {
		if (isset ( $params ['fixturize'] ) && ((is_array ( $params ['fixturize'] ) && ! empty ( $params ['fixturize'] )) || $params ['fixturize'] === true)) {
			...
			if ($controller->uses === false) {
				...
				$list = is_array ( $controller->uses ) ? $controller->uses : array (
						$controller->uses 
				);
			}
	 ...
	 *
	 * @param Controller $controller
	 *        	Controller that has been invoked.
	 * @param array $params
	 *        	parameters as sent by testAction().
	 * @return void
	 * @access public
	 ...
	 *
	 * @param string $url
	 *        	Cake URL to execute (e.g: /articles/view/455)
	 * @param mixed $params
	 *        	Parameters (see above), or simply a string of what to return
	...
	function before($method) {
		...
		// Create records
		if (isset ( $this->_fixtures ) && isset ( $this->db ) && ! in_array ( strtolower ( $method ), array (
				'start',
				'end' 
		) ) && $this->__truncated && $this->autoFixtures == true) {
	...
	function after($method) {
		$isTestMethod = ! in_array ( strtolower ( $method ), array (
				'start',
				'end' 
		) );
	...
	/**
	 * Gets a list of test names.
	 * Normally that will be all internal methods that start with the
	...
	function getTests() {
		return array_merge ( array (
				'start',
				'startCase' 
		), array_diff ( parent::getTests (), array (
				'testAction',
				'testaction' 
		) ), array (
				'endCase',
