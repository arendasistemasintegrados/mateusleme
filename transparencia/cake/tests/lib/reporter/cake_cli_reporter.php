 ...
 */
if (version_compare ( PHP_VERSION, '4.4.4', '<=' ) || PHP_SAPI == 'cgi') {
...
/**
 * Minimal command line test displayer.
 * Writes fail details to STDERR. Returns 0
...
class CakeCliReporter extends CakeBaseReporter {
	...
	function paintException($exception) {
		...
		$message = sprintf ( 'Unexpected exception of type [%s] with message [%s] in [%s] line [%s]', get_class ( $exception ), $exception->getMessage (), $exception->getFile (), $exception->getLine () );
		$message .= $this->_getBreadcrumb ();
