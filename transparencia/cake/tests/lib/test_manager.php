...
class TestManager {
	 ...
	 *        	Reporter object for the tests being run.
	 * @param boolean $testing
	 *        	Are tests supposed to be auto run. Set to true to return testcase list.
	 * @return mixed
	 * @access public
	 */
	 ...
	 *        	Filename of the test to be run.
	 * @param Object $reporter
	 *        	Reporter instance to attach to the test case.
	 * @param boolean $testing
	 *        	Set to true if testing, otherwise test case will be run.
	...
	function runTestCase($testCaseFile, &$reporter, $testing = false) {
		...
		if (! file_exists ( $testCaseFileWithPath ) || strpos ( $testCaseFileWithPath, '..' )) {
			trigger_error ( sprintf ( __ ( "Test case %s cannot be found", true ), htmlentities ( $testCaseFile ) ), E_USER_ERROR );
			return false;
		}
	 ...
	 *        	GroupTest that you want to run.
	 * @param Object $reporter
	...
	function runGroupTest($groupTestName, &$reporter) {
		...
		if (! file_exists ( $filePath ) || strpos ( $filePath, '..' )) {
			trigger_error ( sprintf ( __ ( "Group test %s cannot be found at %s", true ), htmlentities ( $groupTestName ), htmlentities ( $filePath ) ), E_USER_ERROR );
		}
	 ...
	 *
	 * @param object $groupTest
	 *        	Instance of TestSuite/GroupTest that files are to be added to.
	 ...
	 *
	 * @param object $groupTest
	 *        	Instance of TestSuite/GroupTest that a file should be added to.
	 * @param string $file
	 *        	The file name, minus the suffix to add.
	 * @return void
	 * @access public
	 * @static
	 *
	 ...
	 *
	 * @param string $directory
	 *        	Directory to get test case list from.
	 * @access protected
	 */
	 ...
	 *
	 * @param string $directory
	 *        	Directory to get test case files from.
	 * @access protected
	 */
	function &_getTestFileList($directory = '.') {
		$return = $this->_getRecursiveFileList ( $directory, array (
				&$this,
				'_isTestCaseFile' 
		) );
		return $return;
	 ...
	 *
	 * @param string $directory
	 *        	The directory to get group test files from.
	 * @access protected
	 */
	 ...
	 *
	 * @param string $directory
	 *        	The directory to get group tests from.
	 * @access protected
	 */
	 ...
	 *
	 * @param string $groupTestFile
	 *        	The groupTest file to scan for TestSuite classnames.
	 * @access protected
	 */
	 ...
	 *
	 * @param string $type
	 *        	Type of test to get, either 'test' or 'group'
