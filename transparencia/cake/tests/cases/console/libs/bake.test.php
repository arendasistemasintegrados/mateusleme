...
Mock::generatePartial ( 'ShellDispatcher', 'BakeShellMockShellDispatcher', array (
		'getInput',
		'stdout',
		'stderr',
		'_stop',
...
Mock::generatePartial ( 'BakeShell', 'MockBakeShell', array (
		'in',
		'hr',
		'out',
		'err',
...
class BakeShellTestCase extends CakeTestCase {
	...
	function testAllWithModelName() {
		...
		$this->Shell->expectAt ( 2, 'out', array (
				'User Controller was baked.' 
		) );
		$this->Shell->expectAt ( 3, 'out', array (
				'User Views were baked.' 
		) );
		$this->Shell->expectAt ( 4, 'out', array (
				'Bake All complete' 
		) );
