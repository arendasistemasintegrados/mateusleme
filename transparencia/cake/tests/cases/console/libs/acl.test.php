...
Mock::generatePartial ( 'ShellDispatcher', 'TestAclShellMockShellDispatcher', array (
		'getInput',
		'stdout',
		'stderr',
		'_stop',
...
Mock::generatePartial ( 'AclShell', 'MockAclShell', array (
		'in',
		'out',
		'hr',
		'createFile',
...
class AclShellTest extends CakeTestCase {
	...
	function testViewWithModelForeignKeyOutput() {
		...
		$this->Task->expectAt ( 1, 'out', array (
				new PatternExpectation ( '/\[1\] ROOT/' ) 
		) );
		$this->Task->expectAt ( 3, 'out', array (
				new PatternExpectation ( '/\[3\] Gandalf/' ) 
		) );
		$this->Task->expectAt ( 5, 'out', array (
				new PatternExpectation ( '/\[5\] MyModel.2/' ) 
		) );
	...
	function testCreate() {
		...
		$this->Task->expectAt ( 0, 'out', array (
				new PatternExpectation ( '/created/' ),
				'*' 
		) );
		$this->Task->create ();
	...
	function testGrant() {
		...
		$this->Task->expectAt ( 0, 'out', array (
				new PatternExpectation ( '/Permission granted/' ),
				true 
		) );
		$this->Task->grant ();
	...
	function testDeny() {
		...
		$this->Task->expectAt ( 0, 'out', array (
				new PatternExpectation ( '/Permission denied/' ),
				true 
		) );
		$this->Task->deny ();
	...
	function testCheck() {
		...
		$this->Task->expectAt ( 0, 'out', array (
				new PatternExpectation ( '/not allowed/' ),
				true 
		) );
		$this->Task->check ();
		...
		$this->Task->expectAt ( 2, 'out', array (
				new PatternExpectation ( '/is allowed/' ),
				true 
		) );
		$this->Task->check ();
	...
	function testInherit() {
		...
		$this->Task->expectAt ( 1, 'out', array (
				new PatternExpectation ( '/permission inherited/i' ),
				true 
		) );
		$this->Task->inherit ();
	...
	function testInitDb() {
		...
		$this->assertEqual ( $this->Task->Dispatch->args, array (
				'schema',
				'create',
				'DbAcl' 
		) );
