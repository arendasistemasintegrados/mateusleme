...
Mock::generatePartial ( 'ShellDispatcher', 'TestControllerTaskMockShellDispatcher', array (
		'getInput',
		'stdout',
		'stderr',
		'_stop',
...
Mock::generatePartial ( 'ControllerTask', 'MockControllerTask', array (
		'in',
		'hr',
		'out',
		'err',
...
Mock::generatePartial ( 'ModelTask', 'ControllerMockModelTask', array (
		'in',
		'out',
		'err',
		'createFile',
...
Mock::generatePartial ( 'ProjectTask', 'ControllerMockProjectTask', array (
		'in',
		'out',
		'err',
		'createFile',
...
class ControllerTaskTest extends CakeTestCase {
	...
	function testListAll() {
		...
		$this->Task->expectAt ( 1, 'out', array (
				'1. Articles' 
		) );
		$this->Task->expectAt ( 2, 'out', array (
				'2. ArticlesTags' 
		) );
		$this->Task->expectAt ( 3, 'out', array (
				'3. Comments' 
		) );
		$this->Task->expectAt ( 4, 'out', array (
				'4. Tags' 
		) );
		...
		$this->Task->expectAt ( 6, 'out', array (
				'1. Articles' 
		) );
		$this->Task->expectAt ( 7, 'out', array (
				'2. ArticlesTags' 
		) );
		$this->Task->expectAt ( 8, 'out', array (
				'4. Comments' 
		) );
		$this->Task->expectAt ( 9, 'out', array (
				'5. Tags' 
		) );
	...
	function testBakeWithPlugin() {
		...
		$this->Task->expectAt ( 1, 'createFile', array (
				$path,
				new PatternExpectation ( '/ArticlesController extends ControllerTestAppController/' ) 
		) );
		$this->Task->bake ( 'Articles', '--actions--', array (), array (), array () );
	...
	function testBakeActionsUsingSessions() {
		$skip = $this->skipIf ( ! defined ( 'ARTICLE_MODEL_CREATED' ), 'Testing bakeActions requires Article, Comment & Tag Model to be undefined. %s' );
		if ($skip) {
			return;
		}
	...
	function testBakeActionsWithNoSessions() {
		$skip = $this->skipIf ( ! defined ( 'ARTICLE_MODEL_CREATED' ), 'Testing bakeActions requires Article, Tag, Comment Models to be undefined. %s' );
		if ($skip) {
			return;
		}
	...
	function testBakeTest() {
		...
		$this->Task->Test->expectOnce ( 'bake', array (
				'Controller',
				'Articles' 
		) );
	...
	function testInteractive() {
		...
		$this->Task->expectAt ( 0, 'createFile', array (
				$filename,
				new PatternExpectation ( '/class ArticlesController/' ) 
		) );
	}
	...
	function testExecuteIntoAll() {
		$skip = $this->skipIf ( ! defined ( 'ARTICLE_MODEL_CREATED' ), 'Execute into all could not be run as an Article, Tag or Comment model was already loaded. %s' );
		if ($skip) {
			return;
		}
		$this->Task->connection = 'test_suite';
	...
	function testExecuteWithController() {
		$skip = $this->skipIf ( ! defined ( 'ARTICLE_MODEL_CREATED' ), 'Execute with scaffold param requires no Article, Tag or Comment model to be defined. %s' );
		if ($skip) {
			return;
		}
		$this->Task->connection = 'test_suite';
		...
		$this->Task->expectAt ( 0, 'createFile', array (
				$filename,
				new PatternExpectation ( '/\$scaffold/' ) 
		) );
	...
	function testExecuteWithPublicParam() {
		...
		$this->Task->expectAt ( 0, 'createFile', array (
				$filename,
				new NoPatternExpectation ( '/var \$scaffold/' ) 
		) );
	...
	function testExecuteWithControllerAndBoth() {
		...
		$this->Task->expectAt ( 0, 'createFile', array (
				$filename,
				new PatternExpectation ( '/admin_index/' ) 
		) );
