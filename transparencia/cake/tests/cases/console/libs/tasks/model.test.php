...
Mock::generatePartial ( 'ShellDispatcher', 'TestModelTaskMockShellDispatcher', array (
		'getInput',
		'stdout',
		'stderr',
		'_stop',
...
Mock::generatePartial ( 'ModelTask', 'MockModelTask', array (
		'in',
		'out',
		'hr',
		'err',
...
Mock::generate ( 'Model', 'MockModelTaskModel' );

Mock::generate ( 'FixtureTask', 'MockModelTaskFixtureTask' );
...
class ModelTaskTest extends CakeTestCase {
	...
	function testListAll() {
		...
		$this->Task->expectAt ( 5, 'out', array (
				'5. Tag' 
		) );
		...
		$this->Task->expectAt ( 10, 'out', array (
				'4. Comment' 
		) );
		$this->Task->expectAt ( 11, 'out', array (
				'5. Tag' 
		) );
	...
	function testInteractiveFieldValidationWithBogusResponse() {
		...
		$this->Task->expectAt ( 4, 'out', array (
				new PatternExpectation ( '/make a valid/' ) 
		) );
	...
	function testBakeFixture() {
		...
		$this->Task->interactive = true;
		$this->Task->Fixture->expectAt ( 0, 'bake', array (
				'Article',
				'articles' 
		) );
	...
	function testBakeTest() {
		$this->Task->plugin = 'test_plugin';
		$this->Task->interactive = true;
		$this->Task->Test->expectAt ( 0, 'bake', array (
				'Model',
				'Article' 
		) );
	...
	function testInOptions() {
		...
		$this->Task->expectAt ( 0, 'out', array (
				'1. one' 
		) );
		$this->Task->expectAt ( 1, 'out', array (
				'2. two' 
...
STRINGEND;
	...
	function testBakeWithPlugin() {
		...
		$this->Task->expectAt ( 1, 'createFile', array (
				$path,
				new PatternExpectation ( '/Article extends ControllerTestAppModel/' ) 
		) );
		$this->Task->bake ( 'Article', array (), array () );
	...
	function testExecuteWithNamedModel() {
		...
		$this->Task->expectAt ( 0, 'createFile', array (
				$filename,
				new PatternExpectation ( '/class Article extends AppModel/' ) 
		) );
		$this->Task->execute ();
	...
	function testExecuteWithNamedModelHasManyCreated() {
		...
		$this->Task->expectAt ( 0, 'createFile', array (
				$filename,
				new PatternExpectation ( "/'Comment' \=\> array\(/" ) 
		) );
		$this->Task->execute ();
	}
