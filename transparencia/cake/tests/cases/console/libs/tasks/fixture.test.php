...
Mock::generatePartial ( 'ShellDispatcher', 'TestFixtureTaskMockShellDispatcher', array (
		'getInput',
		'stdout',
		'stderr',
		'_stop',
...
Mock::generatePartial ( 'FixtureTask', 'MockFixtureTask', array (
		'in',
		'out',
		'err',
		'createFile',
...
Mock::generatePartial ( 'Shell', 'MockFixtureModelTask', array (
		'in',
		'out',
		'err',
		'createFile',
...
class FixtureTaskTest extends CakeTestCase {
	...
	function testExecuteWithNamedModel() {
		...
		$this->Task->expectAt ( 0, 'createFile', array (
				$filename,
				new PatternExpectation ( '/class ArticleFixture/' ) 
		) );
		$this->Task->execute ();
	}
	...
	function testExecuteIntoAll() {
		...
		$this->Task->Model->setReturnValue ( 'listAll', array (
				'articles',
				'comments' 
		) );
		...
		$this->Task->expectAt ( 1, 'createFile', array (
				$filename,
				new PatternExpectation ( '/class CommentFixture/' ) 
		) );
		$this->Task->execute ();
	}
	...
	function testAllWithCountAndRecordsFlags() {
		...
		$this->Task->expectAt ( 0, 'createFile', array (
				$filename,
				new PatternExpectation ( '/title\' => \'Third Article\'/' ) 
		) );
		
		$filename = '/my/path/comment_fixture.php';
		$this->Task->expectAt ( 1, 'createFile', array (
				$filename,
				new PatternExpectation ( '/comment\' => \'First Comment for First Article/' ) 
		) );
	...
	function testExecuteInteractive() {
		...
		$this->Task->Model->setReturnValue ( 'getTable', 'articles', array (
				'Article' 
		) );
	...
	function testBake() {
		...
		$this->assertPattern ( "/var \\\$import \= array\('records' \=\> true, 'connection' => 'test_suite'\);/", $result );
		$this->assertNoPattern ( '/var \$records/', $result );
		
		$result = $this->Task->bake ( 'Article', 'comments', array (
				'schema' => 'Article' 
		) );
		$this->assertPattern ( "/var \\\$import \= array\('model' \=\> 'Article', 'connection' => 'test_suite'\);/", $result );
		...
		$this->assertPattern ( "/var \\\$import \= array\('model' \=\> 'Article'\, 'records' \=\> true, 'connection' => 'test_suite'\);/", $result );
		$this->assertNoPattern ( '/var \$fields/', $result );
		$this->assertNoPattern ( '/var \$records/', $result );
	}
	...
	function testGenerateFixtureFile() {
		...
		$this->Task->expectAt ( 0, 'createFile', array (
				$filename,
				new PatternExpectation ( '/Article/' ) 
		) );
		$result = $this->Task->generateFixtureFile ( 'Article', array () );
		
		$this->Task->expectAt ( 1, 'createFile', array (
				$filename,
				new PatternExpectation ( '/\<\?php(.*)$/ms' ) 
		) );
		$result = $this->Task->generateFixtureFile ( 'Article', array () );
	}
