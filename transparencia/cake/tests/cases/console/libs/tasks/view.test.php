...
Mock::generatePartial ( 'ShellDispatcher', 'TestViewTaskMockShellDispatcher', array (
		'getInput',
		'stdout',
		'stderr',
		'_stop',
...
Mock::generatePartial ( 'ViewTask', 'MockViewTask', array (
		'in',
		'_stop',
		'err',
		'out',
...
class ViewTaskTest extends CakeTestCase {
	...
	function testExecuteIntoAll() {
		...
		$this->Task->Controller->setReturnValue ( 'listAll', array (
				'view_task_comments' 
		) );
	...
	function testExecuteWithControllerAndAdminFlag() {
		...
		$this->Task->expectAt ( 0, 'createFile', array (
				TMP . 'view_task_articles' . DS . 'admin_index.ctp',
				'*' 
		) );
		$this->Task->expectAt ( 1, 'createFile', array (
				TMP . 'view_task_articles' . DS . 'admin_add.ctp',
				'*' 
		) );
		$this->Task->expectAt ( 2, 'createFile', array (
				TMP . 'view_task_articles' . DS . 'admin_view.ctp',
				'*' 
		) );
		$this->Task->expectAt ( 3, 'createFile', array (
				TMP . 'view_task_articles' . DS . 'admin_edit.ctp',
				'*' 
		) );
