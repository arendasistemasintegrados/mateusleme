...
class XmlHelperTest extends CakeTestCase {
	...
	function testAddNamespace() {
		...
		$expected = array (
				'custom' => 'http://example.com/dtd.xml' 
		);
	...
	function testRemoveNamespace() {
		...
		$expected = array (
				'custom' => 'http://example.com/dtd.xml',
				'custom2' => 'http://example.com/dtd2.xml' 
		);
		$this->assertEqual ( $manager->namespaces, $expected );
	...
	function testRenderZeroElement() {
		...
		$result = $this->Xml->elem ( 'count', null, array (
				'cdata' => true,
				'value' => null 
		) );
		$expected = '<count />';
	...
	function testRenderElementWithNamespace() {
		$result = $this->Xml->elem ( 'count', array (
	...
	function testSerialize() {
		...
		$result = $this->Xml->serialize ( $data, array (
				'format' => 'tags' 
		) );
		...
		$data = array (
				'Pages' => array (
						'id' => 2,
						'url' => 'http://www.url.com/rb/153/?id=bbbb&t=access' 
				) 
		);
		$result = $this->Xml->serialize ( $data );
