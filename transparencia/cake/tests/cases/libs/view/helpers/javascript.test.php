...
class JavascriptTest extends CakeTestCase {
	...
	function testConstruct() {
		$Javascript = & new JavascriptHelper ( array (
				'safe' 
		) );
	...
	function testLink() {
		...
		$result = $this->Javascript->link ( array (
				'prototype.js',
				'scriptaculous.js' 
		) );
	 ...
	 */
	function testValue() {
		$result = $this->Javascript->value ( array (
				'title' => 'New thing',
				'indexes' => array (
						5,
						6,
	...
	function testScriptBlock() {
		...
		$result = $this->Javascript->codeBlock ( 'something', array (
				'allowCache' => true,
				'safe' => false 
		) );
		$expected = array (
		...
		$result = $this->Javascript->codeBlock ( null, array (
				'safe' => true,
				'allowCache' => false 
		) );
		$this->assertNull ( $result );
	...
	function testOutOfLineScriptWriting() {
		echo $this->Javascript->codeBlock ( '$(document).ready(function() { });', array (
				'inline' => false 
		) );
		
		$this->Javascript->codeBlock ( null, array (
				'inline' => false 
		) );
		echo '$(function(){ });';
	...
	function testEvent() {
		...
		$result = $this->Javascript->event ( 'myId', 'click', 'something();', array (
				'safe' => false 
		) );
		$expected = array (
				'script' => array (
		...
		$result = $this->Javascript->event ( '\'document\'', 'load', 'something();', array (
				'safe' => false 
		) );
		$expected = array (
				'script' => array (
