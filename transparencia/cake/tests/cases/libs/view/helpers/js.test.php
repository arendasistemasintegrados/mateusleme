...
Mock::generate ( 'JsBaseEngineHelper', 'TestJsEngineHelper', array (
		'methodOne' 
) );
...
class JsHelperTestCase extends CakeTestCase {
	...
	function testConstruction() {
		$js = & new JsHelper ();
		$this->assertEqual ( $js->helpers, array (
				'Html',
				'Form',
				'JqueryEngine' 
		) );
		
		$js = & new JsHelper ( array (
				'mootools' 
		) );
		$this->assertEqual ( $js->helpers, array (
				'Html',
	...
	function testMethodDispatching() {
		$this->_useMock ();
		$this->Js->TestJsEngine->expectOnce ( 'dispatchMethod', array (
				new PatternExpectation ( '/methodOne/i' ),
				array () 
		) );
	...
	function testMethodDispatchWithBuffering() {
		...
		$this->Js->TestJsEngine->setReturnValue ( 'dispatchMethod', 'This is an event call', array (
				'event',
				'*' 
		) );
		...
		$this->Js->TestJsEngine->setReturnValue ( 'dispatchMethod', 'I am not buffered.', array (
				'effect',
				'*' 
		) );
	...
	function testWriteScriptsNoFile() {
		...
		$view->expectAt ( 0, 'addScript', array (
				new PatternExpectation ( '/one\s\=\s1;\ntwo\s\=\s2;/' ) 
		) );
		$result = $this->Js->writeBuffer ( array (
				'onDomReady' => false,
				'inline' => false,
	...
	function testLinkWithMock() {
		...
		$this->Js->TestJsEngine->expectAt ( 0, 'dispatchMethod', array (
				'get',
		...
		$this->Js->TestJsEngine->expectAt ( 0, 'confirm', array (
				$options ['confirm'] 
		) );
		$this->Js->TestJsEngine->expectAt ( 1, 'request', array (
				'/posts/view/1',
				'*' 
		) );
		$code = <<<CODE
...
CODE;
		$this->Js->TestJsEngine->expectAt ( 1, 'event', array (
				'click',
				$code 
		) );
		$result = $this->Js->link ( 'test link »', '/posts/view/1', $options );
	...
	function testLinkWithNoBuffering() {
		...
		$this->Js->TestJsEngine->setReturnValue ( 'dispatchMethod', '-event handler-', array (
				'event',
				'*' 
		) );
	...
	function testSubmitWithMock() {
		...
		$this->Js->TestJsEngine->setReturnValue ( 'dispatchMethod', 'serialize-code', array (
				'serializeform',
				'*' 
		) );
		$this->Js->TestJsEngine->setReturnValue ( 'dispatchMethod', 'serialize-code', array (
		...
		$this->Js->TestJsEngine->expectAt ( 1, 'dispatchMethod', array (
				new PatternExpectation ( '/serializeForm/i' ),
				'*' 
		) );
		$this->Js->TestJsEngine->expectAt ( 2, 'dispatchMethod', array (
				'request',
		...
		$this->Js->TestJsEngine->expectAt ( 4, 'dispatchMethod', array (
				'get',
				'*' 
		) );
		$this->Js->TestJsEngine->expectAt ( 5, 'dispatchMethod', array (
				new PatternExpectation ( '/serializeForm/i' ),
				'*' 
		) );
		$requestParams = array (
		...
		$this->Js->TestJsEngine->expectAt ( 6, 'dispatchMethod', array (
				'request',
				$requestParams 
		) );
...
class JsBaseEngineTestCase extends CakeTestCase {
	...
	/**
	 * test compatibility of JsBaseEngineHelper::object() vs.
	 * json_encode()
	 *
	 * @return void
	 */
