...
class ViewTest extends CakeTestCase {
	...
	function testViewVars() {
		$this->assertEqual ( $this->View->viewVars, array (
	...
	function testAddInlineScripts() {
		...
		$this->View->addScript ( 'prototype.js' );
		$this->assertEqual ( $this->View->__scripts, array (
				'prototype.js' 
		) );
	...
	function testElement() {
		...
		$result = $this->View->element ( 'plugin_element', array (
				'plugin' => 'test_plugin' 
		) );
	...
	function testElementCacheHelperNoCache() {
		...
		$View = new View ( $Controller );
		$empty = array ();
		$helpers = $View->_loadHelpers ( $empty, array (
				'cache' 
		) );
		$View->loaded = $helpers;
	...
	function testElementCache() {
		...
		$cached = false;
		$result = $View->element ( $element, array (
				'cache' => '+1 second' 
		) );
	...
	function testLoadHelpers() {
		...
		$loaded = array ();
		$result = $View->loadHelpers ( $loaded, array (
				'Html',
				'Form',
				'Ajax' 
		) );
	...
	function testSet() {
		...
		$this->assertIdentical ( $View->viewVars, array (
				'somekey' => 'someValue' 
		) );
		$this->assertIdentical ( $View->getVars (), array (
				'somekey' 
		) );
	...
	function testEntityReference() {
		...
		$View->model = 'Post';
		$View->field = 'title';
		$this->assertEqual ( $View->entity (), array (
				'Post',
				'title' 
		) );
