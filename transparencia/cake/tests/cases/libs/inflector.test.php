...
class InflectorTest extends CakeTestCase {
	...
	function testInflectorSlugWithMap() {
		$result = Inflector::slug ( 'replace every r', array (
				'/r/' => '1' 
		) );
		$expected = '1eplace_eve1y_1';
		$this->assertEqual ( $result, $expected );
		
		$result = Inflector::slug ( 'replace every r', '_', array (
				'/r/' => '1' 
		) );
		$expected = '1eplace_eve1y_1';
		$this->assertEqual ( $result, $expected );
	...
	function testInflectorSlugWithMapOverridingDefault() {
		$result = Inflector::slug ( 'Testing æ ø å', '-', array (
				'/å/' => 'aa',
				'/ø/' => 'oe' 
		) );
	...
	function testCustomTransliterationRule() {
		...
		$this->assertEqual ( Inflector::slug ( 'Testing æ ø å', '-', array (
				'/ø/' => 'oe' 
		) ), 'Testing-ae-oe-aa' );
	}
