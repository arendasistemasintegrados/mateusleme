...
class SessionComponentTest extends CakeTestCase {
	...
	function testSessionReadWrite() {
		...
		$this->assertEqual ( $Session->read ( 'Test.key' ), array (
				'path' => 'some value' 
		) );
		...
		$this->assertFalse ( $Session->write ( array (
				'Test' 
		), 'some value' ) );
	...
	function testSessionFlash() {
		...
		$this->assertEqual ( $Session->read ( 'Message.flash' ), array (
		...
		$this->assertEqual ( $Session->read ( 'Message.myFlash' ), array (
				'message' => 'This is a test message',
				'element' => 'default',
				'params' => array () 
		) );
