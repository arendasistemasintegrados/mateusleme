...
class AuthTest extends CakeTestCase {
	...
	function testIdentifyWithConditions() {
		...
		$this->assertFalse ( $this->Controller->Auth->identify ( $user, array (
				'AuthUser.id >' => 2 
		) ) );
	 ...
	 * not set the session value.
	 * Saving the session value in this circumstance
	...
	function testAllowedActionsSetWithAllowMethod() {
		...
		$this->assertEqual ( $this->Controller->Auth->allowedActions, array (
				'action_name',
				'anotheraction' 
		) );
	}
	...
	function testCustomRoute() {
		Router::reload ();
		Router::connect ( '/:lang/:controller/:action/*', array (
				'lang' => null 
		), array (
				'lang' => '[a-z]{2,3}' 
		) );
		...
		$this->Controller->data ['AuthUser'] = array (
				'username' => 'felix',
				'password' => 'cake' 
		);
	...
	function testCustomField() {
		...
		$this->Controller->data ['AuthUserCustomField'] = array (
				'email' => 'harking@example.com',
				'password' => 'cake' 
		);
		$this->Controller->params ['url'] ['url'] = substr ( $url, 1 );
		$this->Controller->Auth->initialize ( $this->Controller );
		$this->Controller->Auth->fields = array (
				'username' => 'email',
				'password' => 'password' 
		);
		$this->Controller->Auth->loginAction = array (
	...
	function testAjaxLogin() {
		...
		$Dispatcher->dispatch ( '/ajax_auth/add', array (
				'return' => 1 
		) );
		$result = ob_get_clean ();
