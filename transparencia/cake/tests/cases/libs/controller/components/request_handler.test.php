...
App::import ( 'Component', array (
		'RequestHandler' 
) );

Mock::generatePartial ( 'RequestHandlerComponent', 'NoStopRequestHandler', array (
		'_stop',
		'_header' 
) );
Mock::generatePartial ( 'Controller', 'RequestHandlerMockController', array (
		'header' 
) );
...
class RequestHandlerComponentTest extends CakeTestCase {
	 ...
	 */
	function _init() {
		$this->Controller = new RequestHandlerTestController ( array (
				'components' => array (
						'RequestHandler' 
				) 
		) );
	...
	function testDisabling() {
		...
		$this->Controller = new RequestHandlerTestDisabledController ( array (
				'components' => array (
						'RequestHandler' 
				) 
		) );
	...
	function testRespondAs() {
		...
		$RequestHandler->expectAt ( 0, '_header', array (
				'Content-Type: application/json' 
		) );
		$RequestHandler->expectAt ( 1, '_header', array (
	...
	function testRespondAsWithAttachment() {
		...
		$result = $RequestHandler->respondAs ( 'xml', array (
	...
	function testClientContentPreference() {
		...
		$this->assertEqual ( $this->RequestHandler->prefers ( array (
				'js',
				'xml',
				'xhtml' 
		) ), 'xml' );
		...
		$this->assertEqual ( $this->RequestHandler->accepts ( array (
				'js',
				'xml',
				'html' 
		) ), 'xml' );
	...
	function testBeforeRedirectCallingHeader() {
		...
		$controller->expectOnce ( 'header', array (
