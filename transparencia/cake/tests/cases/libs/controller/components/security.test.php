...
class SecurityComponentTest extends CakeTestCase {
	...
	function testRequireLogin() {
		...
		$this->Controller->action = 'posted';
		$this->Controller->Security->requireLogin ( array (
				'posted' 
		), array (
				'type' => 'basic',
				'users' => array (
	...
	function testDigestAuth() {
		$skip = $this->skipIf ( (version_compare ( PHP_VERSION, '5.1' ) == - 1) xor (! function_exists ( 'apache_request_headers' )), "%s Cannot run Digest Auth test for PHP versions < 5.1" );
...
DIGEST;
	...
	function testRequireLoginSettings() {
		$this->Controller->Security->requireLogin ( 'add', 'edit', array (
				'type' => 'basic',
				'users' => array (
						'admin' => 'password' 
				) 
		...
		$this->assertEqual ( $this->Controller->Security->loginUsers, array (
				'admin' => 'password' 
		) );
	}
	...
	function testGenerateDigestResponseHash() {
		...
		$expected = md5 ( md5 ( $data ['username'] . ':' . $loginData ['realm'] . ':' . $data ['password'] ) . ':' . $data ['nonce'] . ':' . $data ['nc'] . ':' . $data ['cnonce'] . ':' . $data ['qop'] . ':' . md5 ( env ( 'REQUEST_METHOD' ) . ':' . $data ['uri'] ) );
