...
class ControllerTest extends CakeTestCase {
	...
	function testPaginate() {
		...
		$Controller->passedArgs = array (
				'sort' => 'ControllerPost.author_id',
				'direction' => 'allYourBase' 
		);
		$results = Set::extract ( $Controller->paginate ( 'ControllerPost' ), '{n}.ControllerPost.id' );
		$this->assertEqual ( $Controller->ControllerPost->lastQuery ['order'] [0], array (
				'ControllerPost.author_id' => 'asc' 
		) );
		$this->assertEqual ( $results, array (
				1,
				3,
	...
	function testPaginateExtraParams() {
		...
		$this->assertEqual ( $Controller->params ['paging'] ['ControllerPost'] ['page'], 1 );
		$this->assertEqual ( Set::extract ( $result, '{n}.ControllerPost.id' ), array (
				1,
				2,
				3 
		) );
		...
		$this->assertEqual ( $Controller->ControllerPost->lastQuery ['conditions'], array (
				'ControllerPost.id > ' => '1' 
		) );
	...
	function testPaginateOrderVirtualField() {
		...
		$result = $Controller->paginate ( 'ControllerPost' );
		$this->assertEqual ( Set::extract ( $result, '{n}.ControllerPost.offset_test' ), array (
				4,
				3,
				2 
		) );
	...
	function testPaginateOrderVirtualFieldSharedWithRealField() {
		...
		$Controller->paginate = array (
				'fields' => array (
						'ControllerComment.id',
						'title',
						'ControllerPost.title' 
				) 
		);
		$Controller->passedArgs = array (
				'sort' => 'ControllerPost.title',
				'dir' => 'asc' 
		);
		$result = $Controller->paginate ( 'ControllerComment' );
		$this->assertEqual ( Set::extract ( $result, '{n}.ControllerComment.id' ), array (
				1,
				2,
				3,
				4,
	...
	function testRedirect() {
		...
		Mock::generatePartial ( 'Controller', 'MockController', array (
				'header' 
		) );
		...
		foreach ( $codes as $code => $msg ) {
			...
			$MockController->expectAt ( 0, 'header', array (
					"HTTP/1.1 {$code} {$msg}" 
			) );
			$MockController->expectAt ( 1, 'header', array (
		...
		$MockController->Component->init ( $MockController );
		$MockController->expectAt ( 0, 'header', array (
				'Location: http://www.example.org/users/login' 
		) );
		...
		$MockController = & new MockController ();
		$MockController->components = array (
				'MockTest' 
		);
		$MockController->Component = & new Component ();
		$MockController->Component->init ( $MockController );
		...
		$MockController->expectAt ( 1, 'header', array (
				'Location: http://book.cakephp.org' 
		) );
		$MockController->expectCallCount ( 'header', 2 );
		$MockController->redirect ( 'http://cakephp.org', 301, false );
		...
		$MockController->expectAt ( 1, 'header', array (
				'Location: http://bakery.cakephp.org' 
		) );
		$MockController->expectCallCount ( 'header', 2 );
		$MockController->redirect ( 'http://cakephp.org', 301, false );
	}
	...
	function testStartupProcess() {
		Mock::generatePartial ( 'AnotherTestController', 'MockedController', array (
				'beforeFilter',
				'afterFilter' 
		) );
		Mock::generate ( 'TestComponent', 'MockTestComponent', array (
				'startup',
				'initialize' 
		) );
		$MockedController = & new MockedController ();
		$MockedController->components = array (
				'MockTest' 
		);
