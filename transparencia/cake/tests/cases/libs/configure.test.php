...
class ConfigureTest extends CakeTestCase {
	...
	function testStoreAndLoad() {
		...
		$expected = array (
				'data' => array (
						'first' => 'value with backslash \, \'singlequote\' and "doublequotes"',
						'second' => 'value2' 
				),
				'data2' => 'value' 
		);
...
class AppImportTest extends CakeTestCase {
	...
	function testClassLoading() {
		...
		$file = App::import ( 'Core', array (
				'NonExistingPlugin.NonExistingModel' 
		), false );
		$this->assertFalse ( $file );
		
		$file = App::import ( 'Core', array (
				'NonExistingPlugin.NonExistingModel.AnotherChild' 
		), false );
		$this->assertFalse ( $file );
	...
	function testFileLoadingReturnValue() {
		...
		$type = array (
				...
				'name' => 'OtherName',
				'parent' => false,
				'file' => TEST_CAKE_CORE_INCLUDE_PATH . 'config' . DS . 'config.php',
				'return' => true 
		);
		$file = App::import ( $type );
		$this->assertTrue ( $file );
	...
	function testLoadingVendor() {
		...
		ob_start ();
		$result = App::import ( 'Vendor', 'TestPlugin.TestPluginAsset', array (
				'ext' => 'css' 
		) );
		$text = ob_get_clean ();
		$this->assertTrue ( $result );
		...
		ob_start ();
		$result = App::import ( 'Vendor', 'TestAsset', array (
				'ext' => 'css' 
		) );
		$text = ob_get_clean ();
		$this->assertTrue ( $result );
		...
		ob_start ();
		$result = App::import ( 'Vendor', 'SomeName', array (
				'file' => 'some.name.php' 
		) );
		$text = ob_get_clean ();
		$this->assertTrue ( $result );
