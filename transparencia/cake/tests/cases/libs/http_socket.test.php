...
class HttpSocketTest extends CakeTestCase {
	...
	function setUp() {
		if (! class_exists ( 'MockHttpSocket' )) {
			Mock::generatePartial ( 'TestHttpSocket', 'MockHttpSocket', array (
					'read',
					'write',
					'connect' 
			) );
			Mock::generatePartial ( 'TestHttpSocket', 'MockHttpSocketRequests', array (
					'read',
					'write',
					'connect',
					'request' 
	...
	function testRequest() {
		...
		$this->Socket->expect ( 'write', array (
				"GET / HTTP/1.1\r\nHost: www.cakephp.org\r\nConnection: close\r\nUser-Agent: CakePHP\r\n\r\n" 
		) );
	...
	function testGet() {
		...
		$this->RequestSocket->expect ( 'request', a ( array (
				'method' => 'GET',
	...
	function testPostPutDelete() {
		...
		foreach ( array (
			...
			$this->RequestSocket->{low ( $method )} ( 'http://www.google.com/', array (
					'Foo' => 'bar' 
			) );
			...
			$this->RequestSocket->{low ( $method )} ( 'http://www.google.com/', null, array (
					'line' => 'Hey Server' 
			) );
		}
	}
	...
	function testParseResponse() {
		...
		$r = $this->Socket->parseResponse ( array (
				'foo' => 'bar' 
		) );
	...
	function testDecodeChunkedBody() {
		...
		$r = $this->Socket->decodeChunkedBody ( $encoded );
		$this->assertIdentical ( $r ['body'], $decoded );
		$this->assertIdentical ( $r ['header'], array (
				'Foo-Header' => 'bar',
				'Cake' => 'PHP' 
		) );
	...
	function testParseHeader() {
		...
		$r = $this->Socket->parseHeader ( array (
				'foo' => 'Bar',
				'fOO-bAr' => 'quux' 
		) );
		$this->assertIdentical ( $r, array (
