...
Mock::generatePartial ( 'BehaviorCollection', 'MockModelBehaviorCollection', array (
		'cakeError',
		'_stop' 
) );
...
class BehaviorTest extends CakeTestCase {
	...
	function testBehaviorBinding() {
		...
		$this->assertEqual ( array_keys ( $Apple->Behaviors->Test->settings ), array (
				'Apple' 
		) );
		...
		$this->assertEqual ( $Apple->Sample->Behaviors->Test->settings ['Sample'], array (
				'beforeFind' => 'on',
				'afterFind' => 'off',
				'key2' => 'value2' 
		) );
		...
		$this->assertIdentical ( $Apple->Sample->Behaviors->Test->settings, $Apple->Behaviors->Test->settings );
		...
		$this->assertEqual ( $Apple->Behaviors->Test->settings ['Apple'], array (
				'beforeFind' => 'off',
				'afterFind' => 'off',
				'key' => 'value',
				'key2' => 'value2',
	...
	function testInvalidBehaviorCausingCakeError() {
		...
		$Apple->Behaviors->expectAt ( 0, 'cakeError', array (
				'missingBehaviorFile',
				'*' 
		) );
	...
	function testBehaviorSaveCallbacks() {
		...
		$expected = Set::merge ( $record, array (
				'Sample' => array (
		...
		$expected = Set::merge ( $record2, array (
				'Sample' => array (
	...
	function testBehaviorValidateCallback() {
		...
		$this->assertIdentical ( $Apple->validationErrors, array (
				'name' => true 
		) );
	...
	function testBehaviorValidateMethods() {
		...
		$this->assertEqual ( array_keys ( $result ['Apple'] ), array (
				'name',
				'color',
				'modified',
				'created' 
	...
	function testBehaviorTrigger() {
		...
		$Apple->beforeTestResult = array ();
		$Apple->Behaviors->trigger ( $Apple, 'beforeTest', array (), array (
				'break' => true,
				'breakOn' => 'test2behavior' 
		) );
		$expected = array (
				'testbehavior',
				'test2behavior' 
