<?php
/* SVN FILE: $Id: model_integration.test.php,v 1.3 2013/08/21 19:19:09 dbrafael.nery Exp $ */
...
class ModelIntegrationTest extends BaseModelTest {
	...
	function testGetAssociated() {
		...
		$this->assertEqual ( $Article->getAssociated ( 'hasMany' ), array (
				'Comment',
				'Category' 
		) );
		
		$results = $Article->getAssociated ();
		$this->assertEqual ( sort ( array_keys ( $results ) ), array (
				'Category',
				'Comment',
				'Tag' 
		) );
	...
	function testConstruct() {
		...
		$this->assertEqual ( $TestModel->actsAs, array (
				'Containable',
				'Tree' 
		) );
