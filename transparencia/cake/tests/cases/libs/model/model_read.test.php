...
class ModelReadTest extends BaseModelTest {
	...
	function testGroupBy() {
		...
		$isStrictGroupBy = in_array ( $db->config ['driver'], array (
				'postgres',
				'oracle' 
		) );
	...
	function testPreparedQuery() {
		...
		$result = $Article->query ( $query, $params );
		$this->assertTrue ( is_array ( $result ) );
		$this->assertTrue ( isset ( $result [0] [$this->db->fullTableName ( 'articles', false )] ['title'] ) || isset ( $result [0] [0] ['title'] ) );
	...
	function testGenerateFindList() {
		...
		$result = Set::combine ( $TestModel->find ( 'all', array (
				'order' => 'Article.title ASC',
				'fields' => array (
						'id',
						'title' 
