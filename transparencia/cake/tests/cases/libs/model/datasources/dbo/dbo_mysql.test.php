...
Mock::generatePartial ( 'DboMysql', 'QueryMockDboMysql', array (
		'query',
		'execute' 
) );
...
class DboMysqlTest extends CakeTestCase {
	...
	function testIndexOnMySQL4Output() {
		...
		$mockDbo->setReturnValue ( 'query', $columnData, array (
	...
	function testVirtualFieldSeparators() {
		...
		$result = $this->db->fields ( $model, null, array (
				'data',
				'other__field' 
		) );
		$expected = array (
	...
	function testSimpleDeleteConditionsNoJoins() {
		$model = & new Post ();
		$mockDbo = & new QueryMockDboMysql ( $this );
		$mockDbo->expectAt ( 0, 'execute', array (
				new PatternExpectation ( '/AS\s+`Post`\s+WHERE\s+`Post/' ) 
		) );
		$mockDbo->setReturnValue ( 'execute', true );
		
		$mockDbo->delete ( $model, array (
				'Post.id' => 1 
		) );
	}
	...
	function testDeleteWithJoins() {
		...
		$mockDbo = & new QueryMockDboMysql ( $this );
		$mockDbo->expectAt ( 0, 'execute', array (
				new PatternExpectation ( '/LEFT JOIN `authors`/' ) 
		) );
		$mockDbo->setReturnValue ( 'execute', true );
	...
	function testDeleteWithJoinsAndMultipleConditions() {
		...
		$mockDbo->expectAt ( 1, 'execute', array (
				new PatternExpectation ( '/LEFT JOIN `authors`/' ) 
		) );
		$mockDbo->setReturnValue ( 'execute', true );
