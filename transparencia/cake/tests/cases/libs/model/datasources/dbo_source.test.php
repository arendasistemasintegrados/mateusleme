 ...
 */
	...
	function testFieldDoubleEscaping() {
		$config = array_merge ( $this->__config, array (
				'driver' => 'test' 
		) );
		$test = & ConnectionManager::create ( 'quoteTest', $config );
		$test->simulated = array ();
		
		$this->Model = & new Article2 ( array (
				'alias' => 'Article',
				'ds' => 'quoteTest' 
		) );
	...
	function testGenerateInnerJoinAssociationQuery() {
		...
		$this->testDb->read ( $this->Model, array (
				'recursive' => 1 
		) );
	...
	function testGenerateAssociationQueryHasManyWithLimit() {
		...
		$result = $this->testDb->generateAssociationQuery ( $this->Model, $params ['linkModel'], $params ['type'], $params ['assoc'], $params ['assocData'], $queryData, $params ['external'], $resultSet );
		$this->assertPattern ( '/^SELECT\s+' . '`TestModel6`\.`id`, `TestModel6`\.`test_model5_id`, `TestModel6`\.`name`, `TestModel6`\.`created`, `TestModel6`\.`updated`\s+' . 'FROM\s+`test_model6` AS `TestModel6`\s+WHERE\s+' . '`TestModel6`.`test_model5_id`\s+=\s+\({\$__cakeID__\$}\)\s*' . 'LIMIT \d*' . '\s*$/', $result );
		
		$result = $this->testDb->generateAssociationQuery ( $this->Model, $null, null, null, null, $queryData, false, $null );
		$this->assertPattern ( '/^SELECT\s+' . '`TestModel5`\.`id`, `TestModel5`\.`test_model4_id`, `TestModel5`\.`name`, `TestModel5`\.`created`, `TestModel5`\.`updated`\s+' . 'FROM\s+`test_model5` AS `TestModel5`\s+WHERE\s+' . '(?:\()?\s*1 = 1\s*(?:\))?' . '\s*$/', $result );
	}
	...
	function testRenderStatement() {
		...
		$result = $this->testDb->renderStatement ( 'update', array (
				'fields' => 'value=2',
				'table' => 'table',
				'conditions' => 'WHERE 1=1',
				'alias' => '' 
		...
		$result = $this->testDb->renderStatement ( 'delete', array (
				'fields' => 'value=2',
				'table' => 'table',
				'conditions' => 'WHERE 1=1',
				'alias' => '' 
	...
	function testStatements() {
		...
		$result = $this->testDb->update ( $Article, array (
				'field1' 
		), array (
				'value1' 
		) );
		...
		$result = $this->testDb->insertMulti ( 'articles', array (
				'field' 
		), array (
				'(1)',
				'(2)' 
	...
	function testMagicMethodQuerying() {
		$result = $this->testDb->query ( 'findByFieldName', array (
				'value' 
		), $this->Model );
		$expected = array (
				'first',
		...
		$result = $this->testDb->query ( 'findByFindBy', array (
				'value' 
		), $this->Model );
		$expected = array (
				'first',
		...
		$result = $this->testDb->query ( 'findAllByFieldName', array (
				'value' 
		), $this->Model );
		$expected = array (
				'all',
		...
		$result = $this->testDb->query ( 'findAllById', array (
				'a' 
		), $this->Model );
		$expected = array (
				'all',
	...
	function testOrderParsing() {
		...
		$result = $this->testDb->order ( array (
				"title desc, id desc" 
		) );
		$this->assertPattern ( '/^\s*ORDER BY\s+`title`\s+desc,\s+`id`\s+desc\s*$/', $result );
	...
	function testCalculations() {
		...
		$result = $this->testDb->calculate ( $this->Model, 'count', array (
				'id' 
		) );
		...
		$result = $this->testDb->calculate ( $this->Model, 'max', array (
				'id' 
		) );
		...
		$result = $this->testDb->calculate ( $this->Model, 'min', array (
				'`Model`.`id`',
				'id' 
		) );
	...
	function testValue() {
		...
		$result = $this->testDb->value ( array (
				'first',
				2,
				'third' 
		) );
	...
	function testRealQueries() {
		...
		$expected = array (
				$this->db->fullTableName ( 'apples', false ) => array (
						'color' => 'Red 1',
						'name' => 'Red Apple 1' 
				) 
		);
	...
	function testVirtualFields() {
		...
		$Article->virtualFields = array (
				'this_moment' => 'NOW()',
				'two' => '1 + 1',
				'comment_count' => 'SELECT COUNT(*) FROM ' . $this->db->fullTableName ( 'comments' ) . ' WHERE Article.id = ' . $this->db->fullTableName ( 'comments' ) . '.article_id' 
		);
	...
	function testVirtualFieldsInCalculate() {
		...
		$result = $this->db->calculate ( $Article, 'count', array (
				'this_moment' 
		) );
		$expected = 'COUNT(NOW()) AS `count`';
		$this->assertEqual ( $expected, $result );
		
		$result = $this->db->calculate ( $Article, 'max', array (
				'comment_count' 
		) );
	...
	function testVirtualFieldsFetch() {
		...
		$query = 'SELECT ' . join ( ',', $this->db->fields ( $Article, null, array (
				'id',
				'comment_count' 
