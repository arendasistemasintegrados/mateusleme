<?php
/* SVN FILE: $Id: model_write.test.php,v 1.3 2013/08/21 19:19:09 dbrafael.nery Exp $ */
...
class ModelWriteTest extends BaseModelTest {
	...
	function testAutoSaveUuid() {
		...
		$this->assertEqual ( array_keys ( $result ['Uuid'] ), array (
				'id',
				'title',
				'count',
				'created',
	...
	function testCounterCacheWithSelfJoin() {
		...
		$result = Set::extract ( $Category->find ( 'all', array (
				'conditions' => array (
						'CategoryThread.id' => 5 
				) 
	...
	function testSaveField() {
		...
		$Node->set ( 'id', 1 );
		$result = $Node->read ();
		$this->assertEqual ( Set::extract ( '/ParentNode/name', $result ), array (
				'Second' 
		) );
	...
	function testSaveWithNullId() {
		...
		$this->assertTrue ( $User->save ( array (
				'password' => 'test' 
		) ) );
	...
	function testUpdateMultiple() {
		...
		$this->assertTrue ( $result );
		$result = Set::extract ( $TestModel->find ( 'all', array (
				'conditions' => array (
						'Comment.user_id' => 5 
				) 
		) ), '{n}.Comment.comment' );
	...
	function testSaveAllHasOneValidation() {
		...
		$this->assertFalse ( $model->saveAll ( array (
				'Comment' => array (
						'comment' => '',
						'article_id' => 1,
						'user_id' => 1 
	...
	function testUpdateWithCalculation() {
		...
		$result = Set::extract ( '/DataTest/count', $model->find ( 'all', array (
				'fields' => 'count' 
		) ) );
		$this->assertEqual ( $result, array (
				5,
	...
	function testUpdateAllWithJoins() {
		$this->skipIf ( $this->db->config ['driver'] == 'postgres', '%s Currently, there is no way of doing joins in an update statement in postgresql' );
		$this->loadFixtures ( 'ProductUpdateAll', 'GroupUpdateAll' );
		$ProductUpdateAll = & new ProductUpdateAll ();
