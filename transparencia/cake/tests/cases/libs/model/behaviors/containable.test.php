...
class ContainableBehaviorTest extends CakeTestCase {
	...
	function testContainments() {
		...
		$this->assertEqual ( Set::extract ( '/ArticleFeatured/keep/Featured/fields', $r ), array (
				'id' 
		) );
		...
		$this->assertEqual ( Set::extract ( '/Article/keep/Comment/fields', $r ), array (
				'comment',
				'published' 
		) );
		$this->assertEqual ( Set::extract ( '/Article/keep/User/fields', $r ), array (
				'user' 
		) );
		$this->assertTrue ( Set::matches ( '/Comment/keep/Attachment', $r ) );
		$this->assertEqual ( Set::extract ( '/Comment/keep/Attachment/fields', $r ), array (
				'attachment' 
		) );
		...
		$this->assertEqual ( array_keys ( $r ), array (
				'Comment',
				'Article' 
		) );
		$this->assertEqual ( array_shift ( Set::extract ( '/Comment/keep', $r ) ), array (
				'keep' => array () 
		) );
		$this->assertTrue ( Set::matches ( '/Article/keep/Comment', $r ) );
		$this->assertEqual ( array_shift ( Set::extract ( '/Article/keep/Comment/.', $r ) ), array (
				'limit' => 1 
		) );
		...
		$this->assertEqual ( array_shift ( Set::extract ( '/User/keep', $r ) ), array (
				'keep' => array () 
		) );
		$this->assertEqual ( array_shift ( Set::extract ( '/Comment/keep', $r ) ), array (
				'keep' => array (
		...
		$this->assertEqual ( array_shift ( Set::extract ( '/Article/keep', $r ) ), array (
				'keep' => array (
						'Comment' => array () 
				) 
		) );
		
		$r = $this->__containments ( $this->Tag, array (
				'Article' => array (
						'User' => array (
								'Comment' => array (
										'Attachment' => array (
		...
		$this->assertEqual ( $r ['Comment'] ['keep'] ['Attachment'] ['conditions'], array (
				'Attachment.id >' => 1 
		) );
	...
	function testFindEmbeddedThirdLevelNonReset() {
		...
		$this->__assertBindings ( $this->User, array (
				'hasMany' => array (
						'ArticleFeatured' 
				) 
		) );
		$this->__assertBindings ( $this->User->ArticleFeatured, array (
				'hasOne' => array (
						'Featured' 
				) 
		) );
		$this->__assertBindings ( $this->User->ArticleFeatured->Featured, array (
				'belongsTo' => array (
						'Category' 
				) 
		) );
		...
		$this->__assertBindings ( $this->User->ArticleFeatured->Comment, array (
				'belongsTo' => array (
						'Article',
						'User' 
				),
		...
		$this->__assertBindings ( $this->User->Article, array (
				'belongsTo' => array (
						'User' 
				),
				'hasMany' => array (
	...
	function testPaginate() {
		...
		$Controller->Article->unbindModel ( array (
				'hasMany' => array (
						'Comment' 
				),
				'belongsTo' => array (
		...
		$Controller->Article->bindModel ( array (
				'hasMany' => array (
						'Comment' 
				),
				'belongsTo' => array (
