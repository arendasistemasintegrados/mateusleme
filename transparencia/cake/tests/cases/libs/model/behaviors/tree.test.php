...
class NumberTreeTest extends CakeTestCase {
	...
	function testAddOrphan() {
		...
		$result = $this->Tree->find ( null, array (
				'name',
				$parentField 
		), $modelClass . '.' . $leftField . ' desc' );
		$expected = array (
				$modelClass => array (
						'name' => 'testAddOrphan',
						$parentField => null 
	...
	function testMovePromote() {
		...
		$direct = $this->Tree->children ( $parent_id, true, array (
				'id',
				'name',
				$parentField,
				$leftField,
	...
	function testMoveWithWhitelist() {
		...
		$result = $this->Tree->children ( $parent_id, true, array (
				'id',
				'name',
				$parentField,
				$leftField,
	...
	function testArraySyntax() {
		...
		$this->Tree = & new $modelClass ();
		$this->Tree->initialize ( 3, 3 );
		$this->assertIdentical ( $this->Tree->childCount ( 2 ), $this->Tree->childCount ( array (
				'id' => 2 
		) ) );
		$this->assertIdentical ( $this->Tree->getParentNode ( 2 ), $this->Tree->getParentNode ( array (
				'id' => 2 
		) ) );
		$this->assertIdentical ( $this->Tree->getPath ( 4 ), $this->Tree->getPath ( array (
				'id' => 4 
		) ) );
	}
}
...
class ScopedTreeTest extends NumberTreeTest {
	...
	function testMoveUpWithScope() {
		...
		$this->assertEqual ( Set::extract ( '/Ad/id', $result ), array (
				6,
				5 
		) );
		$this->assertEqual ( Set::extract ( '/Campaign/id', $result ), array (
				2,
				2 
		) );
	}
