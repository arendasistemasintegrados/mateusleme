<?php
/* SVN FILE: $Id: model_validation.test.php,v 1.3 2013/08/21 19:19:08 dbrafael.nery Exp $ */
...
class ModelValidationTest extends BaseModelTest {
	...
	function testValidatesWithAssociations() {
		...
		$expectedError = array (
				'doomed' => 'This field cannot be left blank' 
		);
	...
	function testValidatesWithModelsAndSaveAllWithoutId() {
		...
		$Article->create ();
		$result = $Article->saveAll ( $data, array (
				'validate' => 'only' 
		) );
		$this->assertTrue ( $result );
		...
		$id = $Article->id;
		$count = $Article->find ( 'count', array (
				'conditions' => array (
						'Article.id' => $id 
				) 
		) );
