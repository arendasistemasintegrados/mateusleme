...
class CakeTestCaseTest extends CakeTestCase {
	...
	function testGetTests() {
		...
		$this->assertEqual ( array_slice ( $result, 0, 2 ), array (
				'start',
				'startCase' 
		) );
		$this->assertEqual ( array_slice ( $result, - 2 ), array (
				'endCase',
				'end' 
		) );
	}
	...
	function testTestAction() {
		App::build ( array (
				...
				'controllers' => array (
						TEST_CAKE_CORE_INCLUDE_PATH . 'tests' . DS . 'test_app' . DS . 'controllers' . DS 
				) 
		), true );
		...
		$result = $this->Case->testAction ( '/tests_apps/some_method', array (
				'return' => 'result' 
		) );
		...
		$this->assertEqual ( array_keys ( $result ['params'] ['named'] ), array (
				'var1',
				'var2' 
		) );
		...
		$this->assertEqual ( $result ['params'] ['pass'], array (
				'gogo',
				'val2' 
		) );
		...
		$this->assertEqual ( array_keys ( $result ['data'] ), array (
				'name',
				'pork' 
		) );
		$fixture->drop ( $db );
	...
	function testTestDispatcher() {
		...
		$return = $Dispatcher->dispatch ( '/tests_apps/index', array (
				'autoRender' => 0,
				'return' => 1,
				'requested' => 1 
		) );
