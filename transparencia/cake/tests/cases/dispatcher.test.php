...
class DispatcherTest extends CakeTestCase {
	...
	function testAdminDispatch() {
		...
		$this->assertIdentical ( $controller->passedArgs, array (
				'param' => 'value',
				'param2' => 'value2' 
		) );
	...
	function testAutomaticPluginControllerDispatch() {
		...
		$this->assertEqual ( $controller->params ['named'], array (
				'param' => 'value',
				'param2' => 'value2' 
		) );
		...
		$this->assertEqual ( $controller->params ['pass'], array (
				5 
		) );
		$this->assertEqual ( $controller->params ['named'], array (
				'param' => 'value',
		...
		$controller = $Dispatcher->dispatch ( 'admin/articles_test', array (
				'return' => 1 
		) );
	...
	function testTestPluginDispatch() {
		...
		$url = '/test_plugin/tests/index';
		$result = $Dispatcher->dispatch ( $url, array (
				'return' => 1 
		) );
	...
	function testAssets() {
		...
		App::build ( array (
				...
				'vendors' => array (
						TEST_CAKE_CORE_INCLUDE_PATH . 'tests' . DS . 'test_app' . DS . 'vendors' . DS 
				),
				'views' => array (
	...
	/**
	 * test that cached() registers a view and un-registers it.
	 * Tests
	...
	function testBasePathInjection() {
		...
		$_SERVER ['PHP_SELF'] = urldecode ( "/index.php/%22%3E%3Ch1%20onclick=%22alert('xss');%22%3Eheya%3C/h1%3E" );
	...
	function testEnvironmentDetection() {
		...
		$this->__loadEnvironment ( array_merge ( array (
				'reload' => true 
		), $backup ) );
	}
	...
	function testTrailingSlash() {
		...
		Router::reload ();
		$Dispatcher = & new TestDispatcher ();
		Router::connect ( '/myalias/:action/*', array (
