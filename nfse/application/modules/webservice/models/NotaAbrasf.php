...
class WebService_Model_NotaAbrasf {
  ...
  protected function escreverListaServico(DOMElement $oNodeServico) {
    ...
    $sCodigoCnaeNota = $oNota->getS_dados_cod_cnae();
    $sCodigoCnae     = '';
    if (!empty($sCodigoCnaeNota)) {

      $sCodigoCnae  = substr($sCodigoCnaeNota, 1, 4)."-";
      $sCodigoCnae .= substr($sCodigoCnaeNota, 5, 1)."/";
      $sCodigoCnae .= substr($sCodigoCnaeNota, 6, 2);
      if (substr($sCodigoCnaeNota, 8, 2) != '') {
        $sCodigoCnae .= "-".substr($sCodigoCnaeNota, 8, 2);
      }

    }

    $oXmlItemListaServico = $oXml->createElement("ii:ItemListaServico", $oNota->getS_dados_item_lista_servico());
    $oXmlCodigoCnae       = $oXml->createElement("ii:CodigoCnae", $sCodigoCnae);
  ...
  protected function escreverTomadorServico() {
    ...
    $oXmlCnpj                 = $oXml->createElement("ii:Cnpj", $oNota->getTomadorCpfCnpj());
