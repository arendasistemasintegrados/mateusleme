...
class WebService_Model_Ecidade {
  ...
  public static function consultar($metodo, $arguments) {
    ...
    try {
      ...
      $a                  = self::getSoapClient()->consultar($metodo, array($arguments[0],$aParametrosGlobais), $arguments[1]);

      self::finishLogging($metodo, $arguments, $iTempoOcorrido);
      $a = self::decode($a);

      if (empty($a)) {
        return 'Não houve retorno do WebService';
      }
  ...
  public static function processar($metodo, $valores) {
    ...
    try {
      ...
      $a                  = self::getSoapClient()->processar($metodo, array($valores, $aParametrosGlobais));
  ...
  public function attr($attr) {
    ...
    if (!property_exists($this->oModelo, $attr)) {
      return NULL;
    }
